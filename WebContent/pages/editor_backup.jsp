	...
	if(isDrawing){
		...
		if(currentObjId != 7){
			...
			if(objectTable != null){
				...
				console.log("selected object::"+selObjArray);
				if(selObjArray!= null){
...
function showSelectedObject(objectSel){
	...
	if(isRotaionEnable && !(objectSel.type == 12 ||objectSel.type == 8)){
...
function setObjectAttributeMenuOption(graphicsObj){
	if(graphicsObj.type != 8){
		console.log("setObjectAttributeMenuOption::"+graphicsObj.fillColor+":::"+graphicsObj.isFilled);
...
function deletelecturerecording(){
	jQuery.ajax({
			...
			success:function(msg) {
			//console.log("-------deletelecturerecording------");
			if(msg.actionErrors != null){
				alert("error");
				return;
			}else{
...
function deleteSlide(){
	//alert("deleteslide");
	var pageNum = document.getElementById("pagenum").value;
	console.log("pageObjTable before:;"+pageObjTable);
	...
	console.log("nonRecordinPageObjTable After:;"+nonRecordinPageObjTable);
	timeRefTable.moveFirst();
	while(timeRefTable.next()){
		var objRecTime = timeRefTable.getKey();
		var recordedObjArr = timeRefTable.getValue();
