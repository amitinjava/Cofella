...
<html>
 <head>
  ...
  var attachNodeId = null;
  var attachUserId = null;
  //var prevId = null;
 ...
 */			var phone = "";
			...
			str = str +  '<li id="'+ obj.id +'"><img src ="images/meeting/lastli.gif" /> <img id="img'+obj.id+'"  src ="images/meeting/ak_plus.gif" style="cursor:pointer" onclick= getChild("'+obj.id +'","getChild") /> <img src ="images/meeting/li.gif" /><span title="Id:'+ obj.id +' order :'+obj.displayOrder + '" ondblclick="editName(this,'+obj.id +')" onclick=attachNodeName("'+obj.nodeName+'",'+obj.id +')> '+obj.nodeName +'</span>&nbsp; <img src ="images/meeting/ak_plus.gif" onclick= addChild("'+obj.id +'") /><input type="hidden" id="'+nodeId+'" name="" value= "'+obj.nodeName+'"><input type="hidden" id="'+orderId+'" name="" value= "'+obj.displayOrder+'"><input type="hidden" id="'+nodeTypeId+'" name="" value= "'+obj.nodeType+'"><input type="hidden" id="'+metaTagId+'" name="" value= "'+metaDataVal+'"><input type="hidden" id="'+ivrOptionId+'" name="" value= "'+obj.ivrOption+'">&nbsp;&nbsp;<i class="icon-remove-sign" onclick="deleteNode('+obj.id +');" ></i></li>';
		});
	...
	function setValues(){
		attachUserId = "";
		var e = document.getElementById("userIds");
		var strUser = e.options[e.selectedIndex].value;
		var strUser1 = e.options[e.selectedIndex].text;
		document.getElementById("userLst").innerHTML = strUser1;
		attachNodeId =strUser;
		
		
	}
	
	function attachNodeName(name,id){
		attachNodeId ="";
		//alert(id+"::::"+name);
		document.getElementById("nodeLst").innerHTML = name;
		attachNodeId =id;
		
	}
	
	 function attachNOdeWithUser(){
		 var data = "userId="+attachNodeId+"&nodeId="+attachNodeId;
			  jQuery.ajax({
			  		type:	"post",
			  		url: 	"attachnode.action",
			  		data: 	data,
			  		success:function(msg) {
			  			//alert(msg.parentId);
 ...
 <div style="float:left;width:100%;padding:0px 20px;border:1px">
 ...
 <div style="float:left;width:40%;border:1px">
   <s:if test="childDTO != null">
	  <ul>
	  	<li id='<s:property	value="childDTO.id" />'>
	  		<img src ="images/meeting/lastli.gif" /><img id='img<s:property	value="childDTO.id" />' src ="images/meeting/ak_plus.gif" style="cursor:pointer"  onclick= "getChild('<s:property value="childDTO.id" />','getChild')" /><img src ="images/meeting/li.gif" /> <span title="order : <s:property	value="childDTO.displayOrder" />" onlclick="editName(this,'<s:property	value="childDTO.id" />')" onclick="attachNodeName('<s:property	value="childDTO.id" />','<s:property value="childDTO.nodeName" />')" ><s:property value="childDTO.nodeName" /></span><input type="hidden" id='nodetype<s:property	value="childDTO.id" />' value = '<s:property	value="childDTO.nodeType" />' name="" style="width: 200px;">&nbsp; <img src ="images/meeting/ak_plus.gif" onclick= "addChild('<s:property value="childDTO.id" />')" />
	  	</li>
	  </ul>
	  <script type="text/javascript">
	 <!--
  ...
  <TABLE border="1" cellpadding="10" cellspacing="10">
	<TR>
		<TH colspan="3" align="center"> Attch User</TH>
	...
	<TR>
		<TD colspan="3" align="center"><input type="button" value="Attach User" /> </TD>
  ...
  <input type = "button" value ="getComplaintByUserId" onclick="findComplaintByUserId()" />
   ...
   <input type = "button" value ="UpdateUser" onclick="UpdateUser()"/>
    ...
    <div style="float:left;width:30%;border:1px">Users
    ...
    <s:if test="userList !=null && userList.size() >0">
		<table>
		 <tr>
			<td colspan="4">
		 		<s:select id="userIds" name="valueId" value="valueId"  list="userList"  
				headerKey="-1"  listKey="userId"  
				listValue="name"  onchange="setValues(this)">
				</s:select>
			</td>
	 	</tr>
