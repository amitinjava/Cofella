<!--
	...
	jQuery(document).ready(function(){
		...
		timeLineDivWidth = timeLineDiv.getBoundingClientRect().width;
	...
	//	sliderDiv.innerHTML = '<input id="ex1" data-slider-id="ex1Slider" type="text" data-slider-min="0" data-slider-max="'+(windowW - 90)+'" data-slider-step="1" data-slider-value="14" style="width:'+(windowW - 90)+'px;"/>';
		...
		if(isMobDevice){
			...
			sliderLength = (timeLineDivWidth - 20);
			//alert("timeLineDivHeight::"+timeLineDivHeight);
			//console.log.log.log("timeln_canvas.width::"+timeln_canvas.width);
		...
		//console.log.log.log("timelnOffsetX"+timelnOffsetX+":timelnOffsetY"+timelnOffsetY);
		initPlayer();
		/*
		...
		*/
		...
		jQuery("#ex2").on('slideStop', function (e) {
			//console.log.log.log("-----------slideStop-------------"+volSliderPos);
			//myVid.volume = (volSliderPos/100);
		});
		...
		jQuery("#ex1").on('slideStop', function (e) {
			//console.log.log.log("-----------slideStop-------------"+sliderPos);
			if(!isRecRunning){
				//console.log.log.log("----runningTimerId---After---"+runningTimerId);
				//console.log.log.log(" clearTimeout(sliderTimer)222222222");
				//console.log.log.log("----runningTimerId---Before---"+runningTimerId);
				...
				stopSlide();
				playingFrom = 2;
				//console.log.log.log("Source::::"+myVid.src);
				if(myVid.src == null || myVid.src == ''){
					...
					//console.log.log.log("-----------currentAudioTime-------"+currentAudioTime);
		...
		canvas.addEventListener("touchstart", function (evt) {
			...
			//console.log.log.log("Touch Start------------------"+touch.clientX+"::Y::"+touch.clientY);
			evt.preventDefault();
			dragFlag = true;
		...
		canvas.addEventListener("touchmove", function (evt) {
			var touch = evt.changedTouches[0];
			//console.log.log.log("Touch Move------------------"+touch.clientX+"::Y::"+touch.clientY);
			evt.preventDefault();
			//handleMouseMove(touch);
		}, false);

		canvas.addEventListener("touchend", function (evt) {
			//console.log.log.log("Touch End------------------");
			var touch = evt.changedTouches[0];
			evt.preventDefault();
			dragFlag = false;
		...
		canvas.addEventListener("dragenter", function (evt) {
			//console.log.log.log("Drag Enter------------------");
			evt.preventDefault();
			createDropZone();
		}, false);
		
		canvas.addEventListener("dragover", function (evt) {
			//console.log.log.log("Drag over--------------------");
			evt.preventDefault();
		}, false);
	
		canvas.addEventListener("dragleave", function (evt) {
			//console.log.log.log("Drag Leave---------------------");
			evt.preventDefault();
			clearDropZone();
		}, false);
		...
		canvas.addEventListener("drop", function (evt) {
		  //console.log.log.log("drop--------------- "+evt.dataTransfer.files.length);
		  evt.preventDefault();
			...
			if(files.length == 0){
				...
				for(var i=0;i<files.length;i++){
					...
					if(!isPlayingStoped){
						...
						dropedFileTimeTab.put(file.name,currentMiliSec);
						//console.log.log.log("*****FileName :::"+file.name+":::Time:::"+currentMiliSec);
					 }
					...
					if (typeof FileReader !== "undefined" && file.type.indexOf("image") != -1) {
						...
						// Note: addEventListener doesn't work in Google Chrome for this event
							...
							tempArray = null;
							tempArray =  new Array();
							//console.log.log.log("Temp Array before page load:111::"+tempArray+"::imageX:"+imageX);
							tempArray[0] = imageX;
							//console.log.log.log("Temp Array -----:"+tempArray+"::imageX:"+imageX);
							tempArray[1] = imageY;
							//console.log.log.log("Temp Array $$$$$$$::"+tempArray+"::imageX:"+imageY);
							tempArray[2] = canvas.width - imageX;
							tempArray[3] = canvas.height - (2*imageY);
							//clearDropZone();
							//console.log.log.log("Temp Array before page load:2222::"+tempArray);
		...
		$(document).keydown(function(evt){
			...
			//console.log.log.log("charCode::;"+charCode);
			if(navigator.platform.indexOf("Mac")!=-1){
				...
				if(evt.metaKey && (evt.which == 65)) {
					evt.preventDefault();
					if(currentObjId == 7){
						//console.log.log.log("select All object");
						var pageNum = document.getElementById("pagenum").value;
						var objectTable = pageObjTable.get(pageNum);
				...
				if(evt.ctrlKey && (evt.which == 65)) {
					...
					if(currentObjId == 7){
						...
						selectAllObject(objectTable);
						if(selObjArray.length > 0){
							//console.log.log.log("11111111111");
							enableDisableRelationOptions(true);
						}
					}
					return false;
		...
		$("#timeline_canvas").on('mousedown', function(e) {
			//console.log.log.log("timelnCanvas mousedown");
		}).on('mouseup', function(e) {
			e.preventDefault();
			sliderDrag = false; 
			//console.log.log.log("timelnCanvas mouseup");
			
		}).on('mousemove', function(e) {
			//console.log.log.log("timelnCanvas mousemove");
		...
		$("#canvas").on('mousedown', function (e) {
			...
			/*
			if(dragFlag){
			 //console.log.log.log("MouseDrag-------------------------------------");
			}else{
			 //console.log.log.log("Mousemove#####################################");
			}*/
			handleMouseMove(e);
		...
		e.preventDefault();
			...
			if(objArr.length > 0){
				var graphicsObject = objectTable.get(objArr[0]);			
					//console.log.log.log("objArr:::::"+graphicsObject.ref);
...
function selectedFile(ri){
	//console.log.log.log("document::"+ri);
  ...
  //console.log.log.log("current Accept::"+document.getElementById('myInput').accept);
  $('#myInput').click();
...
function getfile(){
 var selected_file = document.getElementById('myInput').files[0];
	//var selected_file = document.getElementById('myInput').files[0];
	//console.log.log.log("select::1111111111111111"+selected_file);
...
function setSelectedObjectTime(){
	...
	var pageNum = document.getElementById("pagenum").value;
	//console.log.log.log("------setSelectedObjectTime-------------:"+time.trim());
	if(selObjArray.length > 0){
		...
		//console.log.log.log("new time--------------:"+time.trim());
		if(time.trim().length == 0 || time.trim() == 0){
			//console.log.log.log("convert in to non recording object------");
			...
			nonRecordingObjArray[nonRecordingObjArray.length] = objId;
				...
				//console.log.log.log("timeRefTable after inserting object:"+timeRefTable);
				timeRefTable = sortObjectTimeTable(timeRefTable);
				//console.log.log.log("timeRefTable after sorting:"+timeRefTable);
...
function setId(id,state){
	//console.log.log.log("------------Setid---------------"+id);
		...
		switch(id){
		  ...
		  break;
		  case 16:
				//console.log.log.log("inside case 16-----");
				...
				if(isReplaceEnable){
					...
					if(selObjArray != null && selObjArray.length >0){
						...
						var graphicsObject = objectTable.get(selObjArray[0]);
						init();
						//console.log.log.log("::::::graphicsObject::::"+graphicsObject);
						if(graphicsObject.type == 16){
							...
							startX = x;
							startY = y;
							//console.log.log.log("createEditor:"+startX+"::startY:;"+startY);
...
var obj  = document.getElementById("replaceStartB");
	if(obj.className =='fa fa-toggle-up fa-lg'){// replace session starts
	    ...
	    //console.log.log.log("timeStamp::::"+timeStamp);
		//console.log.log.log("replacedObjArr:::"+replacedObjArr);
		//console.log.log.log("timeRefTable:::"+timeRefTable);
		// remove all replaceable object
		removeAllReplaceableObject();
		//console.log.log.log("replacedObjArr:::"+pageObjTable.get(document.getElementById("pagenum").value););
		if(timeStamp != null){
			...
			for(var i=0;i<replacedObjArr.length;i++){
				...
				recordedObjArr[recordedObjArr.length] = pageObject;
				//console.log.log.log("recordedObjArr::::::::::::::::::"+recordedObjArr);
				...
				var nonRecordingObjArray = nonRecordinPageObjTable.get(pageNum);
				//console.log.log.log("Non recorded object"+nonRecordingObjArray);
				if(nonRecordingObjArray != null){
					//console.log.log.log("if case -----"+objId);
					var pos = checkObjInSelList(nonRecordingObjArray,objId);
					//console.log.log.log("if case pos-----"+pos);
					if(pos != -1){
						// Non recorded Object
					...
					//console.log.log.log("nonRecordingObjArray:::"+nonRecordingObjArray);
				}
			}
		}
...
//console.log.log.log("removeAllReplaceableObject replaceableObjArr:::::"+replaceableObjArr);

   for(var i=0;i<replaceableObjArr.length;i++){
		var objId= replaceableObjArr[i];
		var timeStamp = objectTimeTable.get(objId);
		//console.log.log.log("objId:::"+objId+"::timeStamp::"+timeStamp);	
		var pageNum = document.getElementById("pagenum").value;
		if(timeStamp != null){ // recording object
			...
			var nonRecordingObjArray = nonRecordinPageObjTable.get(pageNum);
			//console.log.log.log("Non recorded object before"+nonRecordingObjArray);
			if(nonRecordingObjArray != null){
				//console.log.log.log("if case -----"+objId);
				var pos = checkObjInSelList(nonRecordingObjArray,objId);
				//console.log.log.log("if case pos-----"+pos);
				...
				//console.log.log.log("nonRecordingObjArray::after:"+nonRecordingObjArray);
			}
		
		}
		var objectTable = pageObjTable.get(pageNum);
		objectTable.remove(objId);
		//console.log.log.log("removeAllReplaceableObject objectTable::after:::"+objectTable)
   }
...
function setZoomType(type){
	//console.log.log.log("ZoomType::::"+type);
	zoomType = type;
	if(type == 2){
...
function initPlayer(){
	...
	//console.log.log.log("Can play listener works here ::playingFrom ::"+playingFrom + ":::isPlayingStoped++"+isPlayingStoped);
	//currentObjId = 7;
	
	switch(playingFrom){
				case 1:// Normal play from playing button
					//console.log.log.log("isPlayingStoped:::"+isPlayingStoped);
					if(!isPlayingStoped){
						canvas.style.cursor  = "default";
						//console.log.log.log("Play file name:::"+myVid.src);
						selObjArray =  null;
						selObjArray = new Array();
						selObjRotatable = true;
						//console.log.log.log("@@@@@");
				...
				case 2: // Play when slider moved
					 //console.log.log.log("----Current Audio time In Listner-----------------"+myVid.currentTime);
					
					 ctx.clearRect(0, 0, canvas.width, canvas.height);
					 restore();
					 var playingStartTime = sliderPos*recFileDuration / sliderLength;
					// myVid.currentTime  = currentAudioTime;
					 if(currentAudioTime != null){
						timeRefTable.moveFirst();
						//console.log.log.log("timeRefTable:::::"+timeRefTable);
						var currentPage = document.getElementById("pagenum").value;
						//console.log.log.log("33333333333333333333333");
						...
						while(timeRefTable.next()){
							...
							if(timeRefTable.getKey()<playingStartTime){
								...
								playCount--;
								//console.log.log.log("nextObjTime:::::"+nextObjTime+"::::currentAudioTime:::"+playingStartTime+":::playCount"+playCount);
						...
						//console.log.log.log("::::::::::sliderMoveTime:::::::::"+sliderMoveTime);
						/*
		...
		//alert("playable-----------");
	     ...
	     /* if(!isPlayingStoped && currentAudioTime == null){
				...
				if(timeRefTable.next()){
					...
					//console.log.log.log("Object1:::"+timeRefTable.getKey()+":Time::::"+timeRefTable.getValue().num);
				...
				while(timeRefTable.next()){
					if(timeRefTable.getKey()<currentAudioTime){
						...
						var nextObjTime = timeRefTable.getKey();
						//console.log.log.log("nextObjTime:::::"+nextObjTime+"::::currentAudioTime:::"+currentAudioTime);
	...
	myVid.addEventListener("loadedmetadata", function(_event) {
		//alert("loadedmetadata called");
			//console.log.log.log("loadedmetadata called"+myVid.src);
			...
			var sec = time % 60;
			*/
			//console.log.log.log("loadedmetadata called"+duration);
			...
			switch(playingFrom){
					...
					case 2://  when slider moved
						//console.log.log.log("-----loadedmetadata------sliderPos-------"+sliderPos+"::recFileDuration::"+recFileDuration);
						currentAudioTime  = sliderPos*recFileDuration / sliderLength;
						//console.log.log.log("-----loadedmetadata------currentAudioTime-------"+currentAudioTime);
						currentAudioTime =  Math.ceil(currentAudioTime / 1000);
						myVid.currentTime  = currentAudioTime;
						break;
					case 3:  // when play from shape works
						//console.log.log.log("loadedmetadata editor page");
						if(selObjArray!= null){
							var id = objectTimeTable.get(selObjArray[0]);
							...
							//console.log.log.log("Set current media time"+myVid.currentTime);
			...
			//console.log.log.log("duration:"+recFileDuration);
	
	});
	myVid.addEventListener("play", function(_event) {
	     //console.log.log.log("^^^^^^^^^^^^^^^Play Start---------------");
...
function setPlayerTime(){
	...
	//console.log.log.log(myVid.currentTime);
}
...
function playfromObject(){
	if(!isRecRunning){
		if(selObjArray!= null){
				var id = objectTimeTable.get(selObjArray[0]);
				//console.log.log.log("id:%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"+selObjArray);
				if(id== null){
					alert("------Selcted Object is not playable----------");
				}else{
					playingFrom = 3;
					...
					//console.log.log.log("slider poition::"+sliderPos);
					if(myVid.src == null || myVid.src == ''){
						myVid.src = "http://"+window.location.host+"/"+filename;
	  		...
	  		data: 	"fileName="+fileName,
		  		success:function(msg) {
		  			...
		  			if(msg.actionErrors != null)
		  			{
						...
						var urlToLoad = null;
							...
							/*
								...
								*/
								...
								//console.log.log.log("currentpage ::::"+currentpage);
						...
						//console.log.log.log("44444444444444444444444");
						canvas.style.cursor = "default";
...
function showUploadModal(fileType){
	//console.log.log.log("hiii"+fileType);
...
//console.log.log.log("handleMouseUp:::");
	if(isDrawing){
		...
		if(currentObjId == 4){
			...
			freeHandQdArr = null;
			 ...
			 //console.log.log.log("triEndX::"+triEndX);
			 if(triEndX == null && triEndY == null){
				  triEndX = mouseX;
				  triEndY = mouseY;
				  return;
			 }else{
			 //console.log.log.log("startX::"+startX+"::startY::"+startY+"::mouseX:;"+mouseX+"::mouseY:;"+mouseY);
		...
		//console.log.log.log("tempPointsArray:111111::"+tempPointsArray);
		if(currentObjId != 7){
			count++;
			var isFilled = false;
			if(fillColor != null){
			...
			if(currentObjId != 16){
				//console.log.log.log("tempPointsArray:222222222::"+tempPointsArray);
				...
				//console.log.log.log("objectTable:222222222::"+objectTable);
			}
			if(isReplaceEnable){
			...
			if(objectTable != null && startX != null && startY != null){
				//console.log.log.log("startX::;"+startX+":::startY::"+startY);
				checkObject(objectTable,startX,startY,mouseX,mouseY);
				//console.log.log.log("selected object: mouse up:"+selObjArray);
				if(selObjArray!= null){
		...
		if(!isPlayingStoped ){// Inserting object while playing
				...
				var currentTime = new Date().getTime();
				currentMiliSec = recFileDuration + Math.ceil(currentTime - startTime);
				//console.log.log.log("mouse up::pagenumber:"+document.getElementById("pagenum").value);
				var pageObject = new PageObject(document.getElementById("pagenum").value,count);
		...
		if(currentObjId == 7){
			...
			if(objectTable != null && startX != null && startY != null){
				//console.log.log.log("startX::;"+startX+":::startY::"+startY);
				checkObject(objectTable,startX,startY,mouseX,mouseY);
				//console.log.log.log("checkObject objjjjjjjjjjjjjj"+selObjArray);
				if(selObjArray!= null && selObjArray.length > 0){
					for(var i=0;i<selObjArray.length;i++){
						var graphicsObject = objectTable.get(selObjArray[i]);
						if(!(graphicsObject.type == 8 || graphicsObject.type == 12 || graphicsObject.type == 16)){
							...
							selObjRotatable = false;
							//console.log.log.log("selObjRotatable::4444:::"+selObjRotatable);
						}
						showSelectedObject(graphicsObject);
					}
					//console.log.log.log("4444444"+selObjRotatable);
					enableDisableRelationOptions(true);
				}
			}
			startX = null;
...
/*
//console.log.log.log("isDrawing:::::"+isDrawing+":currentObjId"+currentObjId+":dragFlag:"+dragFlag);
	if (isDrawing) {
		var mouseX = parseInt(e.clientX - offsetX);
		var mouseY = parseInt(e.clientY - offsetY);		
		if(currentObjId == 4){
		...
		if(currentObjId == 7){
			...
			if(dragFlag){
				//console.log.log.log("mouse move::"+strechableObj);
				if(strechableObj.length >0 && strechableObj[0] != null && dragFlag){
					if(isRotaionEnable){
					    removeNonRotationalObjectFromSel();
						//console.log.log.log("selObjArray:::"+selObjArray);
	...
	startY = mouseY;
	var objArr = checkPointsInObject(objectTable,true);
	//console.log.log.log("objArr::::::::::::::"+objArr);
	if(objArr.length != 0){
			selObjArray = null;
			selObjArray = new Array();
		...
		if(isPlayingStoped ){
			...
			//console.log.log.log("objTime:::::"+objTime + "lastDrawnObjTime::"+lastDrawnObjTime);
			if(objTime != null){
			...
			if(objTime ==  null || objTime < lastDrawnObjTime){
				//console.log.log.log("glow objjjjjjjjjjjjjjjjjjjjjjjjj");
				ctx.clearRect(0, 0, canvas.width, canvas.height);
				drawNonRecordingObject(document.getElementById("pagenum").value);
				restoreRecordedObj(lastDrawnObjTime);
				var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
				for(var i=0;i<selObjArray.length;i++){
					var graphicsObject = objectTable.get(selObjArray[i]);
...
function handleMouseDown(e){
	...
	if(currentObjId == 7){
	   ...
	   // case 2: points can be outside of all all object (set start points for rubberband tool)
		...
		var objArr = checkPointsInObject(objectTable,true);
		//console.log.log.log("checkPointsInObject---Selected Object:::"+objArr);
		if(objArr.length != 0){
			if(isEditTimerEnable){
				...
				if(pos != -1){
				    ...
				    if(state){
						//console.log.log.log("Ponts is in selected region----------------------");
		    ...
		    //console.log.log.log("mouse down:::"+rectangleSelObject);
			...
			//console.log.log.log("State:::"+state+"strechableObj::"+strechableObj[1]+"::"+strechableObj.length);
			if(state){
				return;
...
function handleSelectedObject(objectTable){
	...
	if(selObjArray.length >0){
			for(i=0;i<selObjArray.length;i++){
				...
				if(!(graphicsObject.type == 8 || graphicsObject.type == 12 || graphicsObject.type == 16)){
					selObjRotatable = false;
					//console.log.log.log("selObjRotatable::3333:::"+selObjRotatable);
				}
				showSelectedObject(graphicsObject);
				
			}
			//console.log.log.log("33333333");
			enableDisableRelationOptions(true);
	}else{
		initSelectedObjState();
	}
...
function initSelectedObjState(){
	//console.log.log.log("initSelectedObjState called");
	selObjArray =  null;
	selObjArray = new Array();
	selObjRotatable = true;
	strechableObj[0] = null;
...
function sortObjectTimeTable(mergedTimeRefTable){
	//console.log.log.log("timeRefTable::;"+timeRefTable.hash);
	...
	//console.log.log.log("Sort timeArray::"+timeArray);
	for(var i=0;i<timeArray.length;i++){
		//console.log.log.log("timeRefTable::;"+timeArray[i]);
...
function checkAndMergeTimeRefTable(){
	//console.log.log.log("checkAndMergeTimeRefTable:::"+repTimeRefTable);
	//console.log.log.log("Before Merged timeRefTable:::"+timeRefTable);
	if(repTimeRefTable != null){
		checkAndManageObjectInTable();
		//console.log.log.log("checkAndMergeTimeRefTable::2222222:"+repTimeRefTable);
	...
	//console.log.log.log("Afetr Merged timeRefTable:::"+timeRefTable);
}
function addTable(sourceTab,destTab){
	//console.log.log.log("sourceTab::"+sourceTab+"::destTab::"+destTab);
...
function checkAndManageObjectInTable(){
		...
		var recordedObjArr = repTimeRefTable.getValue();
			...
			var pageObj = recordedObjArr[i];
				if(pageObj != null){
					...
					//console.log.log.log("Attach Object Time:::"+objRecTime);
					var prevTime = objectTimeTable.get(objId);
					//console.log.log.log("prevTime:::"+prevTime);
					if(prevTime != null){
						//console.log.log.log("Already recorded object");
						// For Recorded object
	...
	//console.log.log.log("attachObjectInplayingStream objectTimeTable:"+objectTimeTable);
	*/
}
...
function insertObjectInPlayingStream(currentMiliSec,pageObject){
	...
	//console.log.log.log("insertObjectInPlayingStream:::"+repTimeRefTable);
}


	
function createObject(id,startx,starty,mouseX,mouseY,endX,endY,text){
	//console.log.log.log("createObject---------------id----"+id+"startx:"+startx+"::starty::"+starty+"::mouseX::"+mouseX+"::mouseY:;"+mouseY);
	ctx.save();
	ctx.restore();
	//console.log.log.log("createObject:::"+globalAlpha);
	ctx.globalAlpha = 1.0;
	ctx.setLineDash([0]);
	switch(id){
		case 1:
		...
		/* 	pencilPath.strokeWidth = 2;
			...
			createLine(startx, starty, mouseX , mouseY );
			//restore();
			//console.log.log.log("actual Points::"+startx+","+starty+","+mouseX+","+mouseY);
			//ctx.globalAlpha= 0.8;
		...
		case 5:
			...
			if(endX != null && endY != null){
				//console.log.log.log("---createTriangle----"+globalAlpha);
		...
		case 13:
			...
			break;
		case 16:
					//console.log.log.log("Create object :"+text);
					ctx.clearRect(0, 0, canvas.width, canvas.height);
					restore();
					//ctx.save();
					ctx.beginPath();
					ctx.strokeStyle = styleColor;
					ctx.globalAlpha = globalAlpha;
					//console.log.log.log("1111111111111111111111111111");
...
function checkPointsInCanvas(x11,y11,x22,y22){
	...
	//console.log.log.log("x11::"+x11+"::y11::"+y11 +"::x22::"+x22+"::y22::"+y22);
	...
	//console.log.log.log("val:::"+chk1);
	//console.log.log.log("val:::"+chk2);
	//console.log.log.log("val:::"+chk3);
	//console.log.log.log("val:::"+chk4);
...
function restore(){
	...
	//console.log.log.log("44444444444444444444444");
		...
		//console.log.log.log("restore------isReplaceEnable::::"+replaceableObjArr);
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
		//console.log.log.log("restore------objectTable::::"+objectTable);
		if(objectTable != null){
			objectTable.moveFirst();
			while(objectTable.next()){
			//console.log.log.log("key:::"+objectTable.getKey()+ "::rep::"+objectTable.getValue().type);
				 var isObjInSelList = false;
				 if(replaceableObjArr != null){
				   for(var i=0;i<replaceableObjArr.length;i++){
					
					  if(objectTable.getKey() == replaceableObjArr[i]){
						//console.log.log.log("restore------222222222222222:::"+objectTable.getKey());
						 isObjInSelList = true;
					  }
					}
				  }
				  if(!isObjInSelList){
						//console.log.log.log("----222222222222222:::");
		...
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
		//console.log.log.log("restore----------------------Page Number----------------------------------"+document.getElementById("pagenum").value);
		//console.log.log.log("restore--------------------------------------------------------"+objectTable);
		if(objectTable != null){
			objectTable.moveFirst();
			if(strechableObj.length >0 && strechableObj[0] != null){
			 while(objectTable.next()){
			  if(objectTable.getKey() !=  strechableObj[0].id){
					//console.log.log.log("restore$$$$$$$$$$$$");
					drawObject(objectTable,objectTable.getKey(),false);
				}
			  }
			}else{
				while(objectTable.next()){
				 ...
				 if(selObjArray != null){
				  ...
				  if(!isObjInSelList){
				  //console.log.log.log("-111111111111111");
						drawObject(objectTable,objectTable.getKey(),false);
					}
				}
			}
...
function restoreSelectedObject(){
	//console.log.log.log("restore selected Object");
	var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
	//console.log.log.log("restore----------------------Page Number----------------------------------"+document.getElementById("pagenum").value);
	//console.log.log.log("restore--------------------------------------------------------"+objectTable);
	if(objectTable != null){
...
function restoreInsertedObjectInStream(pageNum){
	//console.log.log.log("---------restoreInsertedObjectInStream-----------"+pageNum);	
	if(repTimeRefTable != null){
			repTimeRefTable.moveFirst();
			while(repTimeRefTable.next()){
				var recordedObjArr = repTimeRefTable.getValue();;
...
function changeTextAreaFontType(obj){
	//console.log.log.log("Font type::"+obj.value);
...
function restoreOnlyPlayedObject(){
	...
	if(key != null){
		...
		//console.log.log.log("key:::"+key+"::recordObjArr::"+recordObjArr);
		for(var i=0;i<recordObjArr.length;i++){
			var pageObj = timeRefTable.get(recordObjArr[i]);
			if(pageObj != null){
				var objectTable = pageObjTable.get(pageObj.num);
				ctx.clearRect(0, 0, canvas.width, canvas.height);
				//console.log.log.log("55555555555555555555555555");
...
function restoreAllExceptImage(){
	//console.log.log.log("restore----------------------restoreAllExceptImage-----------");
	...
	if(objectTable != null){
		objectTable.moveFirst();
		 while(objectTable.next()){
			 ...
			 //console.log.log.log("@@@@@@@restore ---------------"+graphicsObject.type);
			  if(graphicsObject.type != 8){
					//console.log.log.log("44444444444444--");
					drawObject(objectTable,objectTable.getKey(),false);
			   }
		}
...
function restoreUnstrechableObject(selObjId){
	//console.log.log.log("restoreUnstrechableObject--------------"+selObjId);
	var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
	...
	if(objectTable != null){
		...
		while(objectTable.next()){
		  if(objectTable.getKey() != selObjId){
			//console.log.log.log("restoreUnstrechableObject$$$$$$$$$$$$"+objectTable.getKey());
			//console.log.log.log("-5555555555555555");
				drawObject(objectTable,objectTable.getKey(),false);
			}
		  }
...
function drawObject(objectTable,id,highlight){
	//console.log.log.log("drawObject:::id:;"+id+"::objectTable::"+objectTable);
	/*
	if(myVid != null){
		myVid.currentTime = 2.564;
		//console.log.log.log("currentime:::;"+myVid.currentTime);
		//console.log.log.log("Start: " + myVid.seekable.start(0)+ " End: " + myVid.seekable.end(0));
	}
	*/
	var graphicsObject = objectTable.get(id);
	if(graphicsObject != null){
	//console.log.log.log("drawObject:::graphicsObject:;"+graphicsObject.type);
	ctx.beginPath();
	if(highlight == 'sel'){
		//ctx.lineWidth= 2;
		//ctx.setLineDash([3]);
		...
		//console.log.log.log("ctx.lineWidth::::::"+ctx.lineWidth);
		ctx.globalAlpha = 1.0;
	   ...
	   //console.log.log.log("drawObject:;"+graphicsObject.color+"::line width:::"+graphicsObject.lineWidth);
	...
	//console.log.log.log("drawObject--"+ctx.lineCap+"::"+ctx.lineJoin);
	...
	switch(graphicsObject.type){
				case 1:
					...
					var pointsArr = graphicsObject.pointsArray;
					//console.log.log.log("Rec Filled State:"+graphicsObject.isFilled);
					
			
					//console.log.log.log("points::"+pointsArr[0]+","+pointsArr[1]+","+pointsArr[2]+","+pointsArr[3]+","+pointsArr[4]+","+pointsArr[5]+","+pointsArr[6]+","+pointsArr[7]);
				...
				case 4:
					...
					//console.log.log.log("Draw Pencil points Array::"+points);
					//drawPoints(points);
					...
					*/
					/*
					var pointsArr = graphicsObject.pointsArray;
					//console.log.log.log("Length::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::"+pointsArr.length);
					for(i=0;(i+3)<pointsArr.length;i = i+2){
						//ctx.beginPath();
						//console.log.log.log("Restore Points::"+pointsArr[i+0]+","+pointsArr[i+1]+","+pointsArr[i+2]+","+ pointsArr[i+3]);
				...
				case 5:
					//ctx.beginPath();
					var pointsArr = graphicsObject.pointsArray;
					//console.log.log.log("Triangle:::"+graphicsObject.isFilled);
				...
				case 8:
					...
					//console.log.log.log("draw Image-----Id ::::"+tempArray);
					isImageLoaded = false;
					if(graphicsObject.imageLoaded){
						...
						//console.log.log.log("image loaded true:"+document.getElementById("pagenum").value+"::Id::"+graphicsObject.id)
				...
				case 16:
					...
					var pointsArr = graphicsObject.pointsArray;
					//ctx.save();
					//console.log.log.log("222222222222222222222"+graphicsObject.text);
				...
				case 23:
					...
					//console.log.log.log("pointsArr[0]::"+pointsArr[0]+"pointsArr[1]::"+pointsArr[1]+"pointsArr[2]::"+pointsArr[2]+"pointsArr[3]::"+pointsArr[3]);
					if(graphicsObject.isFilled == true){
						ctx.fillStyle = graphicsObject.fillColor;
			      		ctx.fill();
						ctx.save();
		...
		//console.log.log.log("Draw Object:::close path::::");
...
function drawPoints(points) {
	...
	pth.simplify();
	 ...
	 //console.log.log.log("drawPoints---------"+drawPoints);
    ctx.lineCap = 'round';
	ctx.lineJoin = 'round';
...
function clock(){
	...
	var pageNumber = document.getElementById("pagenum").value;
	//console.log.log.log("clock  startTime: storePageChangeEvent::"+startTime);
	storePageChangeEvent(pageNumber);
		//console.log.log.log("clock  startTime:::"+startTime);
	//timer = setTimeout('clock()',10);
	//currentMiliSec = currentMiliSec + 10;
}
function init(){
//console.log.log.log("inside init-----------------");
	 startX =  null;
	 startY =  null;
	 triEndX =  null;
	 triEndY =  null;
	...
	//console.log.log.log("Inside init:::"+currentObjId);
	// if(rectangleSelObject != null)
	// if(selObjArray != null && selObjArray.length>0 && !(isRotaionEnable || isZoomEnable) ){
	 if(selObjArray != null && selObjArray.length>0){
		//console.log.log.log("Inside init22222222222:::"+currentObjId);
			rectangleSelObject = null;
			//console.log.log.log("---- selObjArray =  null--55555555555");
			selObjArray =  null;
			selObjArray = new Array();
			selObjRotatable = true;
			ctx.clearRect(0, 0, canvas.width, canvas.height);
	...
	/*	 var str = document.getElementById("big_image").src;
			...
			//console.log.log.log("line width::::::::::::::::::"+str);  */
			/*lineWidth = 2;
	}*/
	//console.log.log.log("333333333333333333333333333333333333333333");
	canvas.style.cursor = "default";
...
function createOval(x1,y1,x2,y2){
		...
		//console.log.log.log("MousePos::x:"+startX+":y:"+startY+":w:"+ (mouseX - startX)+":h:"+(mouseY - startY));
		var w = x2 - x1;
		var h = y2 - y1;
		var centerX  = x1 + w/2
		var centerY  = y1 + h/2
		...
		//console.log.log.log("MousePos::x:"+startX+":y:"+startY+":w:"+ w+":h:"+h);
...
function createOvalNew(x1,y1,x2,y2,x3,y3,x4,y4){
	...
	//console.log.log.log("22222222222222222");
	ctx.moveTo(rx,ry);
...
function createArc(x1,y1,x2,y2,x3,y3){
	//console.log.log.log("Arc created----:");
	ctx.moveTo(x1, y1);
...
function createCircle(x1, y1, x2, y2){
	//console.log.log.log("Circle created----"+x1+"::y1::"+y1 +"::x2::"+x2+"::y2::"+y2);
	var rx = (x1 + x2) / 2;
	var ry = (y1 + y2) / 2;
	//rx = Math.abs(rx);
	//ry = Math.abs(ry);
	...
	//console.log.log.log("radius:::;"+r);
	ctx.arc(x1, y1, r, 0, 2 * Math.PI, false);
	ctx.stroke();
}

function createRectangle(x1,y1,x2,y2){
	//console.log.log.log("x1::"+x1+"::y1::"+y1 +"::x2::"+x2+"::y2::"+y2);
	ctx.rect(x1, y1,x2-x1,y2-y1);
	//console.log.log.log("Rectangle Created");
...
function createSquare(x1,y1,x2,y2,x3,y3,x4,y4){
	//console.log.log.log("x1::"+x1+"y1::"+y1+"x2::"+x2+"y2::"+y2+"x3::"+x3+"y3::"+y3+"x4::"+x4+"y4::"+y4);
	ctx.moveTo(x1,y1);
	ctx.lineTo(x2,y2);
	ctx.lineTo(x3,y3);
	ctx.lineTo(x4,y4);
...
function getEquilateralTriangletThirdPoint(x1,y1,x2,y2){
    ...
    //console.log.log.log("EquilateralTriangle created----"+x1+"::y1::"+y1 +"::x2::"+x2+"::y2::"+y2);
	var Tx = -x1;
	var Ty = -y1;
...
/*function Rotate(pointX, pointY, centerX, centerY, angle) {
 ...
 //console.log.log.log("Cos45:;"+Math.cos(angle));
 //console.log.log.log("Sin45:;"+Math.sin(angle));
...
function Rotate(tx, ty, pointX, pointY, angleRad) {
  ...
  //  //console.log.log.log("angle for rotation::"+angleRad);
    pointX -= tx;
	pointY -= ty;
	...
	//console.log.log.log("cos(angleRad)::"+Math.cos(angleRad));
	//console.log.log.log("sin(angleRad)::"+Math.sin(angleRad));
  // return coordinates relative to top left corner
	pointX = xnew + tx;
	pointY = ynew + ty;
...
function drawArrowhead(x,y,angleRad){
	...
	ctx.save();
	//console.log.log.log("tempX:"+tempX+"tempY::"+tempY);
	ctx.translate(x,y);
	ctx.rotate(angleRad);
    ctx.moveTo(0,0);
    ctx.lineTo(5,15);
...
function stopPlaying(){
	//console.log.log.log("stop playing called");
	if(!isRecRunning){
		//console.log.log.log("stop playing done");
			isPlayingStoped = true;
			 startTime = 0;
			//console.log.log.log(" clearTimeout(sliderTimer)444444444444444");
			//clearTimeout(sliderTimer);
			stopSlide();
			clearTimeout(playerTimer);
			//document.getElementById("cTime").innerHTML = "<b>00:00</b>";
			...
			//console.log.log.log("myVid::::"+myVid);
			myVid.pause();
			myVid.currentTime = 0;
			isPaused = false;
			pointerImg.style.left = -50 +"px";
			...
			//checkAndMergeTimeRefTable();
			var pageNumber = document.getElementById("pagenum").value;
			//console.log.log.log("6666666666666666666666666666666");
			drawNonRecordingObject(pageNumber);
			restoreAllRecordedObj(pageNumber);
			document.getElementById("playpauseBut").className  = "fa fa-lg fa-play-circle";
			jQuery('#ex1').slider('setValue',sliderPos);
...
function togglePlayPause(){
 ...
 //console.log.log.log(":togglePlayPause:selObjArray:"+selObjArray);
 if(!isRecRunning && recFileDuration >0){
	  //console.log.log.log(":::togglePlayPause-------isRecRunning::"+recFileDuration);
...
function moveSlider(){
	 ...
	 var n = new Date();
	 var cTime =  n.getTime();
	 var timeElap = cTime- prevTime;
	 prevTime = cTime;
	//console.log.log.log("start slide::::"+sliderMoveTime+"::timeElap::"+timeElap);
	*/
	//console.log.log.log("start slide::::"+sliderPos);
	jQuery('#ex1').slider('setValue',sliderPos++);
	var lineLength = sliderLength;
	sliderPosition = Math.floor((lineLength * myVid.currentTime * 1000)/recFileDuration);
	//console.log.log.log("timeLinePos::"+timeLinePos);
//	console.log("sliderPosition::"+sliderPosition+"::lineLength:"+lineLength);
	//sliderPosition = sliderPosition + timeLineLeft;
	
	if(sliderPosition >= timeLineLeft){
	//alert("start moving");
		/*
		var x0 = timeLinePos;
		var y0 = timeLineTop - sliderRadius;
		var xe = timeLinePos + 2*sliderRadius;
		...
		//createProgressLine(timeLineLeft,timeLineTop,timeLinePos,timeLineTop,"#ff0000");
	}
...
function pausePlaying(){
	var keys=  timeRefTable.keys;
	//console.log.log.log("-----------------------------------pausePlaying------------------------------------------"+keys);
	isPaused = true;
	myVid.pause();
	clearTimeout(runningTimerId);
	//console.log.log.log(" clearTimeout(sliderTimer)333333333333");	
	//clearTimeout(sliderTimer);
	stopSlide();
	clearTimeout(playerTimer);	
	var currentTime = new Date().getTime();
	nextPlayingObjtime = currentTime - startTime;
	//console.log.log.log("------currentTime---"+currentTime+"----------startTime------------"+startTime+"------nextPlayingObjtime-----"+nextPlayingObjtime);
}


  function playObject(){
  	isPlayingStoped = false;
	//console.log.log.log("--------------isPaused------------------------------------------"+isPaused+"::playFromSlide::"+playFromSlide);
	//console.log.log.log("selObjArray:::::::::::::::"+selObjArray);
	myVid.pause();
	if(isPaused == false){
		if(isPlayFromObject){
			...
			//myVid=document.getElementById("audio1");
			myVid.src = "http://"+window.location.host+"/"+filename;
			//console.log.log.log("myVid.src:::"+myVid.src);
		}
	}else{
		isPaused = false;
		myVid.play();
		...
		if(nextPlayingObjKey != null){
			...
			//console.log.log.log("keys[i]::"+keys[i]+"::keys[i-1]::"+keys[i-1]);
			//console.log.log.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"+nextPlayingObjtime+"------nextPlayingObjKey-------"+nextPlayingObjKey);
			var nextObjRendringTime = (keys[i] - keys[i-1]) - nextPlayingObjtime;
			//console.log.log.log("nextObjRendringTime:::::"+nextObjRendringTime);
...
*/
    ...
    var duration = myVid.duration;
	//console.log.log.log("duration:"+duration);
    //TODO whatever
	});
	*/
	//myVid.src = "http://www.topchalks.com/tc/kina_9.mp3";
	//console.log.log.log("Play file name:::"+filename+".mp3");
...
function restoreAndPlayObject(key){
	...
	//console.log.log.log("restoreAndPlayObject::::"+key);
	...
	if(recordedObjArr != null){
		...
		if(playCount < keys.length){
			...
			//console.log.log.log("timeDuration::::"+keyy);
			nextPlayingObjKey = keyy;
			startTime = new Date().getTime();
			//myVid.currentTime = key;
			//console.log.log.log("-----------Start audio playing-----------");
			//myVid.play();
			var lineLength =  window.innerWidth - 80;
			sliderPos =  Math.floor(key * lineLength/recFileDuration);
			
			//console.log.log.log("^^^^^^^^^^^^^^^^^^^^^^^^sliderPos^^^^^^^^^^^^^^^"+sliderPos);
			//console.log.log.log("-----sliderMoveTime-------"+sliderMoveTime);
			startSlide();
			//console.log.log.log("+++++++++++++++++++++++++++++++++++++++++++++");
...
function checkPageAndDrawObj(key){
	playCount++;
	//console.log.log.log("checkPageAndDrawObj:::"+key);
	var recordedObjArr = timeRefTable.get(key);
	//console.log.log.log("timeRefTable:::"+timeRefTable);
	var pageObj = recordedObjArr[0];
	//console.log.log.log("pageObj:::"+pageObj)
	var objectTable = pageObjTable.get(pageObj.num);
	//console.log.log.log("pageObj.num:::;"+pageObj.num+":::::Pagenumber ::"+document.getElementById("pagenum").value);
	if(pageObj.num == document.getElementById("pagenum").value){
		//console.log.log.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"+recordedObjArr.length);
		for(var i=0;i<recordedObjArr.length;i++){
			...
			if(i == 0){
				...
				if( pointsArray != null){	
				//console.log.log.log("pointerX::"+pointerX+"::pointerY:;"+pointerY+":;pointsArray[0]::"+pointsArray[0]+"::pointsArray[3]:"+pointsArray[3]);
					currentPointerPath = getPointsOnPointerPath(pointerX,pointerY,pointsArray[0],pointsArray[3],50);
					clearTimeout(pointerTimerId);	
					pointCtr = 0;
					//console.log.log.log("currentPointerPath::::"+currentPointerPath);
					drawPointerPath();
				}
			}
			//console.log.log.log("-777777777777777777");
			drawObject(objectTable,pageObj.objectId,false);
		...
		//console.log.log.log("*********************************************"+document.getElementById("pagenum").value);
		document.getElementById("pagenum").value = pageObj.num;
		document.getElementById("pagenumber").innerHTML = pageObj.num;
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		//console.log.log.log("8888888888888888888888888888888");
		drawNonRecordingObject(pageObj.num);
		restoreRecordedObj(key);
		//console.log.log.log("-888888888888888888888");
		drawObject(objectTable,pageObj.objectId,false);
		refreshTimeLine(key);
		lastDrawnObjTime = key;
		pointerImg.style.left = -50 +"px";
...
function restoreRecordedObj(timestamp){
	timeRefTable.moveFirst();
    //console.log.log.log("--------------restoreRecordedObj:::::"+timestamp);
	var currentPage = document.getElementById("pagenum").value;
	var objectTable = pageObjTable.get(currentPage);
	while(timeRefTable.next()){
	//console.log.log.log("key:::::"+timeRefTable.getKey());
		if(parseInt(timeRefTable.getKey())< parseInt(timestamp)){
			var recordedObjectArr = timeRefTable.getValue();
			//console.log.log.log("--------------restoreRecordedObj:::recordedObjectArr::"+recordedObjectArr);
			for(var i=0;i<recordedObjectArr.length;i++){
				var pageObj = recordedObjectArr[i];
				if(pageObj.num == currentPage){
					//console.log.log.log("--------------restoreRecordedObj:::drawObject:Id:"+pageObj.objectId);
					//console.log.log.log("-99999999999999999");
					drawObject(objectTable,pageObj.objectId,false);
				}else{
					continue;
				}
...
function restoreAllRecordedObj(pageNum){
	//console.log.log.log("Page number::::;"+pageNum);
	var objectTable = pageObjTable.get(pageNum);
	timeRefTable.moveFirst();
	while(timeRefTable.next()){
	//console.log.log.log("1111111111111111111111111");
		var recordedObjArr = timeRefTable.getValue();
		for(var i=0;i < recordedObjArr.length;i++){
			var pageObj = recordedObjArr[i];
			if(pageObj.num == pageNum){
					//console.log.log.log("2222222222222222222222222222222");
					//console.log.log.log("-**********************");
				drawObject(objectTable,pageObj.objectId,false);
			}else{
				continue;
...
function drawNonRecordingObject(pageNum){
	...
	if(nonRecordingObjArray != null){
		//console.log.log.log("------------Draw Non Recording Objects---------------------"+nonRecordingObjArray.length);
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		var objectTable = pageObjTable.get(pageNum);
		for(ctr = 0;ctr<nonRecordingObjArray.length;ctr++){
		//console.log.log.log("-$$$$$$$$$$$$$$$$$$$$$$$$$");
...
function checkObject(objectTable,startX,startY,mouseX,mouseY){
	//console.log.log.log("checkObject:::"+startX+","+startY+","+mouseX+","+mouseY);
	objectTable.moveFirst();
	while(objectTable.next()){
	  ...
	  var isSel = false;
	   switch(graphicsObject.type){
				...
				case 2:
					ctx.beginPath();
					var pointsArr = graphicsObject.pointsArray;
					//console.log.log.log("points::::"+pointsArr);
					var rx = (pointsArr[2] - pointsArr[0]) / 2;
				    var ry = (pointsArr[3] - pointsArr[1]) / 2;
				    //console.log.log.log("p1::::"+(pointsArr[0]+rx)+","+ pointsArr[1]);
					//console.log.log.log("p2::::"+(pointsArr[0]+rx)+","+ pointsArr[3]);
					//console.log.log.log("p3::::"+pointsArr[0]+","+ (pointsArr[1]+ry));
					//console.log.log.log("p4::::"+pointsArr[2]+","+ (pointsArr[1]+ry));
...
function closeEditor(isSave){
	//myEditor.saveHTML();	
	var text = document.getElementById("area1").value;
	//console.log.log.log("close Editor::"+document.getElementById("area1").value);
  ...
  /*  var html = myEditor.get('element').value;
	//console.log.log.log("html::"+myEditor.cleanHTML(html));
	while(html.indexOf("&nbsp;")!= -1){
		html = html.replace('&nbsp;',' ');
	}
	//console.log.log.log("remove spaceeee::"+html);
	
	var pos = html.indexOf("style");
	if(pos != -1){
		var styleJson ="{" + html.substring(pos+7,html.indexOf(">")-2) +"}";
		//console.log.log.log("styleJson::"+styleJson);
	
	}
	*/
	//var text = html;
	//console.log.log.log("inside close Editor:::"+text.trim().length);
...
function saveTextAsGraphicsObject(text){
		//console.log.log.log("saveTextAsGraphicsObject::"+text);
		var graphicsObject;
		if(selectedTxtObj != null){
			//console.log.log.log("saveTextAsGraphicsObject:iffff:");
		...
		var pageNum = document.getElementById("pagenum").value;
		var objectTable = pageObjTable.get(pageNum);
		//console.log.log.log("saveTextAsGraphicsObject::objectTable11111111:::"+objectTable);
		if(objectTable == null){
			objectTable = new Hashtable();
			pageObjTable.put(pageNum,objectTable);
		}
		objectTable.put(count, graphicsObject);
		objectPageRefTable.put(count,pageNum);
		//console.log.log.log("saveTextAsGraphicsObject::objectTable22222222222:::"+objectTable);
		selectedTxtObj = null;
}
...
function replaceSelctedObjectWithText(text,startX,startY,objectTable){
	...
	var removeableObjArr;
		  ...
		  //console.log.log.log("replaceableObj::"+replaceableObj.type+"::replaceableObj:;"+replaceableObj.pointsArray);
...
function createText(textObj,startX,startY,color){
	//console.log.log.log("createText text::"+textObj.textData+":::styleColor::"+color+ "textObj.fontSize::"+textObj.fontSize+"::textObj.fontType::"+textObj.fontType);
	ctx.fillStyle = color;
	var decoration = "";
	if(textObj.isItalic){
...
function createGlowText(textObj,startX,startY,color){
	...
	var size = parseInt(textObj.fontSize) + 20;
	...
	//console.log.log.log("selObjArray::::"+selObjArray);
		...
		for(var i=0;i<selObjArray.length;i++){
			...
			for(var j=0;j<pointsArray.length;j++){
				//console.log.log.log("pointsArray::"+pointsArray);
				if(j % 2==0)
		...
		//console.log.log.log("ponitXarr::"+ponitXarr+"::ponitYarr::"+ponitYarr+"::ponitXarr[ponitXarr.length-1]::"+ponitXarr[ponitXarr.length-1]+"::ponitYarr[ponitYarr.length-1]::"+ponitYarr[ponitYarr.length-1]);
...
function point_in_rectagnle(x,y,left, top, right, bottom){
  //console.log.log.log("Dotted Rectangle points::"+left+":"+top+":"+right+":"+bottom);
  result = false;
...
//console.log.log.log("objectTable::"+objectTable+"pageNum::"+pageNum);
 for(var ctr = 0;ctr<selObjArray.length;ctr++){
	//console.log.log.log("selObjArray.length::"+selObjArray.length);
	...
	// remove from non recording object aray
		...
		//console.log.log.log("pos::"+pos);
		var recordObjArr = timeRefTable.get(pos);
		//console.log.log.log("recordObjArr::"+recordObjArr);
		if(recordObjArr != null){
			for(var i=0;i<recordObjArr.length;i++){
			//console.log.log.log("timeRefTable::"+timeRefTable);
				var pageObj = recordObjArr[i];
				//console.log.log.log("pageObj::"+pageObj);
				//console.log.log.log("pageObj ::"+pageObj.objectId+"num::"+pageObj.num);
				if(pageObj.objectId == key){
				//console.log.log.log("object deleted::"+key);
 ...
 ctx.clearRect(0, 0, canvas.width, canvas.height);
 rectangleSelObject = null;
 //console.log.log.log("---- selObjArray =  null--6666666666666");
 selObjArray =  null;
 selObjArray = new Array();
 selObjRotatable = true;
 restore();
...
//console.log.log.log("objectTable::"+selObjRotatable);
  ...
  if(objectTable != null){
   objectTable.moveFirst();
   while(objectTable.next()){
   //console.log.log.log("Type::;"+objectTable.getValue().type);
		var graphicsObject = objectTable.getValue();
...
function stretchSelectedObject(object,mouseX,mouseY){
	...
	//console.log.log.log("Stretch Object---"+selObjArray);
	var graphicsObject = object[0];
	if(graphicsObject.type == 3){
		//console.log.log.log("-----------stretchSelectedObject------------"+graphicsObject.type + " pt1:;"+object[1]+":;pt2::"+object[2]);
		...
		//console.log.log.log("Stretch Object---Triangle");
		...
		showSelectedObject(graphicsObject);
		   ...
		   //console.log.log.log("object[3]:::;"+object[3]);
		...
		if(object[1] == 0 && object[2] == 1){
			//console.log.log.log("1111111:::"+object[1]+","+object[2]);
			//console.log.log.log("1::"+graphicsObject.pointsArray[0]+"::2::"+graphicsObject.pointsArray[1]+"::3::"+graphicsObject.pointsArray[2]+"::4::"+graphicsObject.pointsArray[3]+"::5::"+graphicsObject.pointsArray[4]+"::6::"+graphicsObject.pointsArray[5]+"::7::"+graphicsObject.pointsArray[6]+"::8::"+graphicsObject.pointsArray[7]);
...
//console.log.log.log("translateSelectedObject");
	var dx = x -  startX;
	var dy = y -  startY;
...
function shiftSelectedObject(objectTable,dx,dy){
	//console.log.log.log("-----------------shiftSelectedObject:::"+selObjArray);
	  
	  //console.log.log.log("rectangleSelObject.pointsArray:::"+rectangleSelObject.pointsArray);
		...
		*/
		if(rectangleSelObject != null){
		  ...
		  if(selObjArray!= null){
			  for(var i=0;i<selObjArray.length;i++){
				var graphicsObject = objectTable.get(selObjArray[i]);
				//console.log.log.log("shifted object------"+graphicsObject.id+"type::"+graphicsObject.type);
				var pointsArr = graphicsObject.pointsArray;
					...
					//console.log.log.log("status:::"+status+"::graphicsObject.pointsArray::"+graphicsObject.pointsArray);
					drawObject(objectTable,selObjArray[i],false);
					var graphicsObject = objectTable.get(selObjArray[i]);
					showSelectedObject(graphicsObject);
				}
	...
	function checkAllPointsInCanvas(pointsArr){
		...
		for(var ctr=0;ctr<pointsArr.length;ctr = ctr+2){
					...
					//console.log.log.log("status::"+status+":X:"+pointsArr[ctr]+":Y:"+pointsArr[ctr+1]);
					if(!status){
							break;
						}
		}
...
function checkPointsInObject(objectTable,isSingleRequire){
	...
	if(objectTable != null){
		...
		objectTable.next();
		//console.log.log.log("ObjectTable:::"+objectTable);
		while(objectTable.prev()){
		  ...
		  //console.log.log.log("graphicsObject:::"+graphicsObject);
		   switch(graphicsObject.type){
					...
					case 3:
						var pointsArr = graphicsObject.pointsArray;
						//console.log.log.log("points is::::"+points_is_onLine(startX,startY,pointsArr[0],pointsArr[1],pointsArr[2],pointsArr[3]));						
						break;
					case 4:
						var pointsArray = graphicsObject.pointsArray;
						break;
		  ...
		  //console.log.log.log("graphicsObject:::"+graphicsObject);
			   switch(graphicsObject.type){
						case 1:
							...
							//console.log.log.log("Check points::::"+isPointInPoly(polyArr,pt));
						...
						case 3:
							...
							if(is_in_path(startX, startY, pointsArray,graphicsObject.lineWidth)){
								returnObjArr[returnObjArr.length] = objectTable.getKey();
								//console.log.log.log("Return Object Array::"+returnObjArr);
...
var returnVal = false;
	if(objectTable != null){
	//	//console.log.log.log("-------checkOpenObjectForSelection  called------------------"+objectTable.size());
		objectTable.moveFirst();
		while(objectTable.next()){
			var graphicsObject = objectTable.get(objectTable.getKey());
...
function checkSelectedObjectStrechableState(objectTable,x,y){
     //console.log.log.log("----------------checkSelectedObjectStrechableState-------------"+selObjArray);
	 for(var i=0;i<selObjArray.length;i++){	
		var graphicsObject = objectTable.get(selObjArray[i]);
		var arr = graphicsObject.pointsArray;
		...
		if(graphicsObject.type ==5 || graphicsObject.type == 11 || graphicsObject.type == 13){
			if(is_in_ellipse(x,y,arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius)){
					//console.log.log.log("inside Selection"+canvas.style.cursor);
				...
				if(is_in_ellipse(x,y,arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius)){
					//console.log.log.log("inside Selection 1st"+canvas.style.cursor);
					strechableObj[0] = graphicsObject;
					strechableObj[1] = 0;
					strechableObj[2] = 1;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection 2nd"+canvas.style.cursor);
					strechableObj[0] = graphicsObject;
					strechableObj[1] = 0;
					strechableObj[2] = 3;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection 3rd"+canvas.style.cursor);
					strechableObj[0] = graphicsObject;
					strechableObj[1] = 2;
					strechableObj[2] = 3;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection 4th"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 2;
					strechableObj[2] = 3;
					if(isRotaionEnable){
			...
			if(is_in_ellipse(x,y,arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius)){
				...
				//console.log.log.log("a1::"+arr[0]+"::a2::"+arr[1]+"::b1::"+arr[2]+"::b2::"+arr[3]);
				if(is_in_ellipse(x,y,arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius)){
					//console.log.log.log("inside Selection of point 1"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 0;
					strechableObj[2] = 1;
					strechableObj[3] = 1;
					...
					//console.log.log.log("inside Selection of point 2"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 0;
					strechableObj[2] = 3;
					strechableObj[3] = 2;
					...
					//console.log.log.log("inside Selection of point 3"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 2;
					strechableObj[2] = 1;
					strechableObj[3] = 3;
					...
					//console.log.log.log("inside Selection of point 4"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 2;
					strechableObj[2] = 3;
					strechableObj[3] = 4;
				...
				//console.log.log.log("555555555555555555555555555555");
					canvas.style.cursor  = "default";
					strechableObj[0] = null;
					strechableObj[1] = null;
					strechableObj[2] = null;
				...
				if(is_in_ellipse(x,y,arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius)){
					//console.log.log.log("inside Selection1"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 0;
					strechableObj[2] = 1;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection2"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 2;
					strechableObj[2] = 3;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection3"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 4;
					strechableObj[2] = 5;
					if(isRotaionEnable){
					...
					//console.log.log.log("inside Selection4"+canvas.style.cursor);
					strechableObj[0] = graphicsObject
					strechableObj[1] = 6;
					strechableObj[2] = 7;
					if(isRotaionEnable){
...
function checkPointsInSelectionState(graphicsObject,x,y){
	...
	//console.log.log.log("----------------checkPointsInSelectionState-------------"+graphicsObject);
...
function is_in_Text(tx,ty,graphicsObject){
	...
	var pointsArr = graphicsObject.pointsArray;
			ctx.beginPath();
			ctx.strokeStyle = "red";
			//console.log.log.log("is_in_Text::::::");
			ctx.globalAlpha = 100;
			fontStyle = "30px bethhandfont";
			createText(graphicsObject.text,pointsArr[0],pointsArr[1],graphicsObject.color);
			ctx.stroke();
		
			//console.log.log.log("endTm----"+endTm);
			//console.log.log.log("Time taken---"+(endTm-stTime));
			
			if (ctx.isPointInStroke(tx, ty)){
			 //console.log.log.log("is_in_Text true");
				canvas.style.cursor = "pointer"
				ctx.globalAlpha = currentGAlpha;
				return true;
			}else {
			  //console.log.log.log("is_in_Text false");
				canvas.style.cursor = "default"
				ctx.globalAlpha = currentGAlpha;
				return false;
			}	
...
function is_in_path(tx,ty,pointsArray,strokeWidth){
		var currentGAlpha = ctx.globalAlpha;
			...
			//console.log.log.log("stTime:::"+stTime); 
			...
			if (ctx.isPointInStroke(tx, ty)){
			  ...
			  //console.log.log.log("is_in_path true");
				canvas.style.cursor = "pointer"
				ctx.globalAlpha = currentGAlpha;
				
				return true;
			}else {
			   //console.log.log.log("is_in_path false");
				canvas.style.cursor = "default"
				ctx.globalAlpha = currentGAlpha;
				return false;
			}	
...
function showSelectedObject(objectSel){
	//console.log.log.log("showSelectedObject:::;;"+objectSel);
	...
	if(objectSel.type == 12 ||objectSel.type == 8 || objectSel.type == 16){
		//console.log.log.log("selected----------"+ctx.lineWidth + "::"+ctx.strokeStyle);
	...
	ctx.beginPath();
		...
		//console.log.log.log("objectSel.type:::"+objectSel.type);
		ctx.beginPath();
		ctx.strokeStyle = selectionColor;
		ctx.lineWidth= 2;
		//console.log.log.log("hiiii");
		createOval(arr[0]-selCircleRedius,arr[1]-selCircleRedius,arr[0]+selCircleRedius,arr[1]+selCircleRedius);
		ctx.fill();
		ctx.closePath() ;
		ctx.moveTo(arr[2]+selCircleRedius,arr[3]);
...
function glowSelectedObject(graphicsObject){
	...
	ctx.strokeStyle = "white";
	ctx.lineWidth= parseInt(graphicsObject.lineWidth) + 10;
	//console.log.log.log("glowSelectedObject ::::::"+ctx.lineWidth);
	ctx.globalAlpha = 1.0;
/*	if(objectSel.type == 1){
		//console.log.log.log("hiiiiiiiii");
...
function is_in_ellipse(px,py,x1,y1,x2,y2){
   //console.log.log.log("px:"+px+":py::"+py+":x1:"+x1+":y1:"+y1+":x2:"+x2+"y2:"+y2);
	var w = x2 - x1;
	var h = y2 - y1;
	var centerX  = x1 + w/2
	var centerY  = y1 + h/2
...
function points_is_onLine(x1,y1,x2,y2){
 ...
 /*
  ...
  //console.log.log.log("Triagle area:::"+area);
  return area;
  */
    var angleRad = slope_In_Radian(x1,y1,x2,y2);
	angleDeg = angleRad * 180 / Math.PI;
	...
	angleRad = angleDeg * Math.PI/180;
	//console.log.log.log("angleDeg::;"+angleDeg);
	 y1 = y1+20;
	 y2 = y2+20;
   ...
   //console.log.log.log(":x1r:"+x1r+":y1r:"+y1r+":x2r:"+x2r+":y2r:"+y2r);
...
function distanceBetween(x1,y1,x2,y2){
   ...
   //console.log.log.log("distance:::"+distance);
   return distance
}
...
function changeSelectedObjectColor(color){
		//console.log.log.log("Style color::;"+color);
	  if(selObjArray!= null){
		  //console.log.log.log("ppp:"+document.getElementById("pagenum").value);
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
			  for(var i=0;i<selObjArray.length;i++){
				var graphicsObject = objectTable.get(selObjArray[i]);
				//console.log.log.log("graphicsObject:;"+graphicsObject+":;color:;"+color);
...
function changeSelectedObjectLineWidth(width){
	  ...
	  if(selObjArray!= null){
		...
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
			  for(var i=0;i<selObjArray.length;i++){
				var graphicsObject = objectTable.get(selObjArray[i]);
					//console.log.log.log("graphicsObject:;"+graphicsObject+":;width:;"+width);
...
function changeSelectedObjectGlobalAlpha(globalalpha){
	  ...
	  if(selObjArray!= null){
		...
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
			  for(var i=0;i<selObjArray.length;i++){
				var graphicsObject = objectTable.get(selObjArray[i]);
					//console.log.log.log("current Global Alpha:;"+globalAlpha);
...
function setFillPropOnSelctedObject(fillColor){
	//console.log.log.log("fillColor::"+fillColor);
	  if(selObjArray!= null){
		   ctx.clearRect(0, 0, canvas.width, canvas.height);
		   restore();
			//console.log.log.log("Global Alplha::::"+globalAlpha);
			var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
		//	ctx.save();
			//ctx.closePath();
			ctx.beginPath();
...
//console.log.log.log("rotateSelectedObject::::"+selObjArray)
	if(selObjArray!= null){
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
		...
		var tx = ancPoint.x;
		var ty = ancPoint.y;
		//console.log.log.log("mouseX points::"+mouseX+","+mouseY);
		//console.log.log.log("startX points::"+startX+","+startY);
		var dx = mouseX-startX;
		var dy = mouseY-startY;
		//console.log.log.log("center points::"+dx+","+dy);
		var angleX1 = tx-mouseX;
		var angleY1 = ty-mouseY;
  
		angleR1=Math.atan2(angleY1, angleX1);
		//console.log.log.log("angleR1::"+angleR1);
  
		var angleX2 = angleX1-dx;
		var angleY2 = angleY1-dy;
		angleR2=Math.atan2(angleY2, angleX2);
		//console.log.log.log("angleR2::"+angleR2);
	...
	//console.log.log.log("objectTable--------------------"+objectTable);
		for(var i=0;i<selObjArray.length;i++){
			...
			for(var j=0;j<pointsArray.length;j+=2){
				...
				// //console.log.log.log("Points After Rotation:::"+obj.newX+","+obj.newY);
				if(obj.newX == null){
...
function getRotationAnchorPoint(objectTable,selObjArray){
		//console.log.log.log("graphicsObject"+selObjArray);
			var cx,cy;
			if(selObjArray.length > 1){
				...
				if (graphicsObject.type ==5 || graphicsObject.type ==11){
					...
					//console.log.log.log("points::"+pointsArray[0]+","+pointsArray[1]+","+pointsArray[2]+","+pointsArray[3]+","+pointsArray[4]+","+pointsArray[5]+","+pointsArray[6]+","+pointsArray[7]);
...
function insertImageObj(pageNum,src,imageLoaded,fileName){
	var objectTable = pageObjTable.get(pageNum);
	count++;
	//console.log.log.log("222222222:::"+count);
	...
	objectPageRefTable.put(count,pageNum);
		...
		if(!isPlayingStoped){
				...
				//console.log.log.log("case 1 CurrentTime:"+currentMiliSec);
				//console.log.log.log("case 1 CurrentTime::::"+currentMiliSec+"::::count:::::"+count);
				var pageObject = new PageObject(pageNum,count);
				//objectTimeTable.put(count,imageInsertTime);
				
				//timeRefTable.put(currentMiliSec,pageObject);
				//console.log.log.log("insertObjectInPlayingStream ********"+currentMiliSec);
				insertObjectInPlayingStream(imageInsertTime,pageObject);
				//console.log.log.log("case 1 objectTimeTable:"+objectTimeTable);
			...
			*/
			var imageInsertTime = dropedFileTimeTab.get(fileName);
			//console.log.log.log("****Inserted *FileName :::"+fileName+":::Time:::"+imageInsertTime);
			//console.log.log.log("currentMiliSec::::"+currentMiliSec+"::::count:::::"+count);
			var pageObject = new PageObject(pageNum,count);
			objectTimeTable.put(count,imageInsertTime);
		...
		//	timeRefTable.put(imageInsertTime,pageObject);
			...
			//console.log.log.log("timeRefTable::::"+timeRefTable+"pageObject"+pageObject.num+"::"+pageObject.objectId);
		}else{
			var nonRecordingObjArray = nonRecordinPageObjTable.get(pageNum);
			if(nonRecordingObjArray == null){
				nonRecordingObjArray = new Array();
...
function drawImage(x1,y1,x2,y2){
	//console.log.log.log("--------------------draw   Image-------------------");
...
function loadAndDrawImage(){
	...
	//console.log.log.log("checkImageLoaded:::"+isImageLoaded);
	if(!isImageLoaded){
...
function checkImageLoaded(id){
	isImageLoaded  = imageObj.complete;
	//console.log.log.log("checkImageLoaded:::"+id);
	if(!isImageLoaded){
		  imageloadingTimer = setTimeout("checkImageLoaded("+id+")",1000);
			...
			var pageNum = document.getElementById("pagenum").value;
			var objectTable = pageObjTable.get(pageNum);
			//console.log.log.log("id::"+id+":::pageNum:;"+pageNum);
			var graphicsObject = objectTable.get(id);
			var ptsArray = graphicsObject.pointsArray;
		//clearWaitZone();		
		//console.log.log.log("Image Object------------------------------------"+imageObj.naturalWidth + "::"+imageObj.naturalHeight);
			//console.log.log.log("tempArray--111-----------"+tempArray);
			...
			//console.log.log.log("tempArray---2222----------"+tempArray);
		/*	}else{
		...
		*/
		//console.log.log.log("Image Object- current Height-----------------------------------::"+imageObj.width+":::"+imageObj.height);
...
function zoomImage(imageObject){
	//console.log.log.log("----------zoomimage-----------"+imageObject);
	...
	//console.log.log.log("checkAllPointsInCanvas(pointsArr) ::::;"+state);
	if(state){
		//console.log.log.log("@@@@@@@@@@@pointsArr::::"+pointsArr);
		pointsArray =  null;
		imageObject.pointsArray = pointsArr;
		//console.log.log.log("@@@@@@@@@@@pointsArray::::"+pointsArray);
		//console.log.log.log("@@@@@@@@@@@imageObject.pointsArray::::"+imageObject.pointsArray);
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		restore();
		var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
...
function getImageWidthHeight(imageObj,width,height){
	var  pagenum = document.getElementById("pagenum").value;
	//console.log.log.log("----------------getImageWidthHeight-----canvas---------"+width+"::height:;"+height+"::pagenum::"+pagenum);
	var imageWidth = imageObj.naturalWidth;
	var imageHeight = imageObj.naturalHeight;
	//console.log.log.log("----------------getImageWidthHeight-----image---------"+imageWidth+"::height:;"+imageHeight);
	if(imageWidth <= width && imageHeight <= height){
		...
		if(canvasProportion > imageProportion){
			//console.log.log.log("----------------111111111111111111111111--------------");
			imageObj.height = (height);
			imageObj.width = Math.ceil((imageWidth * imageObj.height) / imageHeight);
		}else{
			//console.log.log.log("----------------22222222222222222222222222----------");
...
function setPageNum(val){
	...
	//console.log.log.log("11111111111111111111111111");
...
function storePageChangeEvent(pagenum){
	count++;
	//console.log.log.log("333333333:::"+count);
	var objectTable = pageObjTable.get(pagenum);
	...
	objectTable.put(count, graphicsObject);
	objectPageRefTable.put(count,pagenum);
	//console.log.log.log("insertObjectInPlayingStream &&&&&currentMiliSec&&&"+currentMiliSec);
	if(!isPlayingStoped){
			currentMiliSec =Math.floor(myVid.currentTime * 1000);
			//console.log.log.log("case 1 CurrentTime:"+currentMiliSec);
			//console.log.log.log("case 1 CurrentTime::::"+currentMiliSec+"::::count:::::"+count);
			...
			//timeRefTable.put(currentMiliSec,pageObject);
			//console.log.log.log("timeRefTable::::"+timeRefTable+"pageObject"+pageObject.num+"::"+pageObject.objectId+"objectTimeTable::"+objectTimeTable);
	}
}
...
function saveMeetingOnServer(eventType){
	...
	//console.log.log.log("jsonData:::"+jsonData);
	 jQuery.ajax({
		  	type:	"POST",
	  		url: 	"save.action",
	  		data: jsonData,
...
function createJSON(){
		...
		//console.log.log.log("recFileDuration:::::"+duration);
	...
	if(pageObjTable.size()>0){
		...
		//console.log.log.log("pageObjTable:::"+pageObjTable);
			...
			if(objectTable.size()>0){
				 ...
				 while(objectTable.next()){
					 ...
					 if(graphicsObject.pointsArray != null){
						 var pointsArray = graphicsObject.pointsArray;
						 var points = '';
						 //console.log.log.log("PonitsArray:::"+pointsArray);
						 for(var i=0;i<pointsArray.length;i++){
								points = points + pointsArray[i] + ',';
								//console.log.log.log("points:::"+points);
							}
					 ...
					 //console.log.log.log(objectTimeTable);
					 if(timeStamp != null){
...
function checkEvent(eType){
	//console.log.log.log("eType::::"+eType);
	if(eType == "open"){
		  		...
		  		success:function(msg) {
		  			...
		  			if(msg.actionErrors != null)
		  			{
		  				alert("error");
		  		  		...
		  		  		//console.log.log.log("showchild"+msg.jsonContent);
						...
						if(json.RecFname!= null){
							...
							//console.log.log.log("Recording recFileName::"+recFileName);
							...
							//console.log.log.log("Recording file name::"+filename);
							
						}
		  		  		initDataStruture();
						//console.log.log.log("--------initDataStruture-----------");
						...
						//document.getElementById("pagenum").value = pageNum;
						//console.log.log.log("22222222222222222222222222222");
						
						loadAllImages();
						
		  		  		drawNonRecordingObject(pageNum);
						restoreAllRecordedObj(pageNum);
						//console.log.log.log("---------------filename----------------------------"+filename);
						if(filename.length >0){
							myVid=document.getElementById("audio1");
							myVid.src = "http://"+window.location.host+"/"+filename;
...
function parseGraphicsIbjectList(pageNum,gObjectList){
	...
	jQuery.each(gObjectList, function(i, obj) {
	    //console.log.log.log("text object:::"+obj.text);
		var textObj;
		if(obj.text != null){
		...
		//console.log.log.log("44444444:::"+count);
		objectTable.put(obj.id, gObj);
		objectPageRefTable.put(obj.id,pageNum);
		//console.log.log.log("obj.timeStamp::::"+obj.timeStamp);
...
function getPointsOnPointerPath(x1,y1,x2,y2,pointsDelta){
	var dx = x2 -x1;
	var dy = y2 -y1;
	//console.log.log.log("dx:::"+dx+"::dy:::"+dy);
...
//console.log.log.log("drawPointerPath::::"+currentPointerPath);
...
function setplayingTimeInPlayer(){
	...
	//console.log.log.log("setplayingTimeInPlayer::"+duration);
/* 
 //console.log.log.log("current audio time::"+myVid.currentTime );
  var audioTime = ""+myVid.currentTime ;
  //console.log.log.log("current audio time::"+audioTime);
  var pos= audioTime.indexOf(".");
  //console.log.log.log("current audio pos::"+pos);
  var timeinSec = audioTime.substring(0,pos);
  var min = parseInt(timeinSec / 60);
  var sec = ''+timeinSec % 60;
  //console.log.log.log("current audio pos::"+sec.length);
  if(sec.length == 1){
	sec = '0'+sec;
  }*/
  document.getElementById("cTime").innerHTML = duration;
  //console.log.log.log("min::"+min+"::sec::"+sec);
...
function startBlink(){
	//console.log.log.log("start blink::");
...
function showRecordingTime(){
	  var currentTime = new Date().getTime();
	 // //console.log.log.log("current startTime ::"+startTime+"::currentTime::"+currentTime);
	  currentMiliSec = recFileDuration + Math.ceil(currentTime - startTime);
	  var duration = getDurationInMinAndSec(currentMiliSec);
	/*
	  var recordTime = ""+(currentMiliSec / 1000);
	 // //console.log.log.log("current recordTime ::"+recordTime);
...
function enableDisableRelationOptions(state){
	//console.log.log.log("enableDisableRelationOptions ::"+state+"::selObjRotatable::"+selObjRotatable);
	if(state){
		//console.log.log.log("hiii");
		//document.getElementById("rotateButton").className = "btn btn-default active"
		if(selObjRotatable ){
			//console.log.log.log("Innnn");
...
function setZoomEnableDisable(state){
	...
	//console.log.log.log("setZoomEnable::2");
	//setZoomType(2);
}
function clearRecordedObjectTime(){
	//console.log.log.log("clearRecordedObjectTime------------------");
	timeRefTable.moveFirst();
	while(timeRefTable.next()){
...
function RemoveRecordedObject(){
	//console.log.log.log("RemoveRecordedObject------------------");
	timeRefTable.moveFirst();
	while(timeRefTable.next()){
		var objRecTime = timeRefTable.getKey();
		var recordedObjArr = timeRefTable.getValue();
...
function editObject(graphicsObject,startx,starty,mouseX,mouseY,endX,endY,text){
	//console.log.log.log("Stroke color::"+ctx.strokeStyle);
	/*
	if(graphicsObject.opacity != null){
		ctx.globalAlpha = graphicsObject.opacity;
	}else{
...
function deletelecturerecording(){
	jQuery.ajax({
			...
			if(msg.actionErrors != null){
				...
				//console.log.log.log("------Successfully Copied------");
			}
	}});
	$('#close-popover').click();	
}
function deleteSlide(){
	...
	//console.log.log.log("pageObjTable After:;"+pageObjTable);
	
	//console.log.log.log("nonRecordinPageObjTable before:;"+nonRecordinPageObjTable);
...
//console.log.log.log("removeNonRotationalObjectFromSel===================="+selObjArray);
	var isRemoved = false;
	var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
	if(selObjArray.length >0){
		for(i=0;i<selObjArray.length;i++){
	...
	//console.log.log.log("removeNonRotationalObjectFromSel========After============"+selObjArray);
}

function closeModal(){
//console.log.log.log("Hiiiii");
...
//console.log.log.log("windowResize================");
	 var windowW=window.innerWidth;
	 var windowH=window.innerHeight
	 canvas.width = (windowW - 60);
	 ...
	 sliderLength = (timeLineDivWidth - 20);
	 //console.log.log.log("sliderLength::;"+sliderLength+"::timeLineDivWidth::"+timeLineDivWidth);
	 timelnCanvasOffset = $("#timeline_canvas").offset();
	 timelnOffsetX = timelnCanvasOffset.left
	 timelnOffsetY = timelnCanvasOffset.top;
	 //console.log.log.log("X::"+timelnOffsetX+"::Y::"+timelnOffsetY);
...
function pasteCopiedObject(){
	...
	selObjArray = null;
	selObjArray = new Array();
	//console.log.log.log("copiedObjArr::"+copiedObjArr);
	var destObjectTable = pageObjTable.get(pageNum);
	//console.log.log.log("objectTable before copying ::;"+destObjectTable+":::count::;"+count);
	for(var i=0;i<copiedObjArr.length;i++){
		count++;
		 ...
		 //console.log.log.log("graphicsObject::"+graphicsObject.type);
	...
	//console.log.log.log("objectTable After copying ::;"+destObjectTable);
	var dx = 10; 
	var dy = 10; 
	//console.log.log.log("pasted object------------"+selObjArray);
...
function attachReference(){
	...
	//console.log.log.log("Ref Ataached::::url::"+refUrl+"::selObjArray::"+selObjArray);
	if(refUrl != ""){
		if(selObjArray != null && selObjArray.length >0){
					var objectTable = pageObjTable.get(document.getElementById("pagenum").value);
					for(var i=0;i<selObjArray.length;i++){
						var graphicsObject = objectTable.get(selObjArray[0]);
						//console.log.log.log("Ref Ataached::"+graphicsObject.id+"::url::"+refUrl);
...
function getMinMax(pointsArray){
	...
	ponitXarr = ponitXarr.sort(function(a, b){return a-b});	
	ponitYarr = ponitYarr.sort(function(a, b){return a-b})
	//console.log.log.log("Xarray::"+ponitXarr);	
	//console.log.log.log("YArray::"+ponitYarr);	
...
function exSmooth_1(ps) { 
	// //console.log.log.log("exSmooth:::"+ps);
	  var a = 0.2; 
...
function exSmooth(x1,y1,x2,y2) { 
	 //console.log.log.log("exSmooth:::x::"+x1+":::y::"+y1);
	  var a = 0.2; 
	  /*
	  var x1 = ps[ps.length-2];
	  var y1 = ps[ps.length-1];
	 ...
	 /* ps[ps.length-2] = x1 * a + x2 * (1-a);
	  ps[ps.length-1] = y1 * a + y2 * (1-a);
  */
	//console.log.log.log("x::"+(x1 * a + x2 * (1-a))+"::y::"+(y1 * a + y2 * (1-a)));
     return {x:x1 * a + x2 * (1-a), y:y1 * a + y2 * (1-a)} 
}

 function drawFreeHandLine(points) { 
 //console.log.log.log("drawLine::;"+points);
  ctx.moveTo(points[0], points[1]); 
...
function openVolume(){
	//console.log.log.log("Hiiii"+document.getElementById("volCtrl").style.display);
...
function convertImgToBase64(imgObj, outputFormat){
  //console.log.log.log("convertImgToBase64---------------------");
    var img = new Image;
	...
	img.onload = function(){
		...
		//console.log.log.log("dataUrl:::"+dataURL);
...
function checkAllImagesLoaded(){
	...
	//console.log.log.log("checkAllImagesLoaded::::"+allImagesLoaded);
	if(!allImagesLoaded){
...
function startObjectPlaying(){
	...
	startSlide();
	//alert("start");
	//console.log.log.log("------------startObjectPlaying----------------");
	checkAndMergeTimeRefTable();
	timeRefTable = sortObjectTimeTable(timeRefTable);
	//console.log.log.log("3333333333333333333333333333"+timeRefTable);
	drawNonRecordingObject("1");
	timeRefTable.moveFirst();
	//console.log.log.log("timeRefTable::::playing:"+timeRefTable);
	playCount = 0;
	if(timeRefTable.next()){
		//alert(timeRefTable.getKey());
		//alert(timeRefTable.getValue());
	...
	//console.log.log.log("----1111-recFileDuration-------"+recFileDuration+"::sliderLength::"+sliderLength);
}
...
function createTimeLine(){
	//TimeLineObject
	//objectTime = -1;
	//console.log.log("createTimeLine::::");
	...
	timelnCtx.lineWidth= 2;
	timelnCtx.moveTo(timeLineLeft, timeLineTop);
    timelnCtx.lineTo(lineLength, timeLineTop);
	//console.log.log("lineLength"+lineLength+"::timeLineLeft::"+timeLineLeft);
	timelnCtx.stroke();
	...
	while(timeRefTable.next()){
		...
		for(var i=0;i<recordedObjArr.length;i++){
			var pageObj = recordedObjArr[i];
			var objId = pageObj.objectId;
			//console.log.log.log("objId:::"+objId);
			var pageNum = pageObj.num;
			var objectTable = pageObjTable.get(pageNum);
			var graphicsObj = objectTable.get(objId);
			//console.log.log.log("graphicsObj Type::"+graphicsObj.type+"recFileDuration::"+recFileDuration);
			var timeLinePos = Math.floor((lineLength * objTime)/recFileDuration);
			if(timeLinePos == 0){
				timeLinePos = timeLineLeft;
			}
			...
			if(graphicsObj.type == 10){
				type = 1;
				var x1 = timeLinePos - timeLinePageObjRadius;
				var y1 = timeLineTop + timeLinePageObjRadius;
				...
				timelnCtx.lineTo(x1,(y1- 2*timeLineObjRadius));
				timelnCtx.lineTo((x1+2*timeLineObjRadius),(y1- 2*timeLineObjRadius));
				timelnCtx.lineTo((x1+2*timeLineObjRadius),y1);
				*/
				styleColor = "#000000";
				createTimeLineOval(ptsArray[2],ptsArray[3],ptsArray[6],ptsArray[7],styleColor,pageNum);
			}else{
				/*var x1 = timeLinePos-timeLineObjRadius;
				var y1 = timeLineTop + timeLineObjRadius;
				ptsArray[ptsArray.length] = x1;
				ptsArray[ptsArray.length] = y1;
				ptsArray[ptsArray.length] = x1;
				ptsArray[ptsArray.length] = (y1- 2*timeLineObjRadius);
				ptsArray[ptsArray.length] = (x1+2*timeLineObjRadius);
				ptsArray[ptsArray.length] = (y1- 2*timeLineObjRadius);
				ptsArray[ptsArray.length] = (x1+2*timeLineObjRadius);
				ptsArray[ptsArray.length] = y1;
				*/
				...
				ptsArray[ptsArray.length] = x1;
				ptsArray[ptsArray.length] = y1;
				ptsArray[ptsArray.length] = timeLinePos ;
				ptsArray[ptsArray.length] = (timeLineTop - timeLineObjRadius);
				ptsArray[ptsArray.length] = (timeLinePos + timeLineObjRadius);
				ptsArray[ptsArray.length] = y1;
				ptsArray[ptsArray.length] = (timeLinePos);
				ptsArray[ptsArray.length] = (timeLineTop +timeLineObjRadius);
				...
				type = 2;
				styleColor = "#00FFFF";
				createTimeLineSquare(ptsArray[0],ptsArray[1],ptsArray[2],ptsArray[3],ptsArray[4],ptsArray[5],ptsArray[6],ptsArray[7],styleColor);
...
function refreshTimeLine(objectTime){
	//alert("refreshTimeLine:::");
	//console.log.log("refreshTimeLine::"+objectTime+":sliderPosition:;"+sliderPosition);
	if(objectTime != -1){
	...
	//console.log.log.log("lastDrawnObjTime:;"+lastDrawnObjTime+"::w::"+objectTime);
	...
	if(sliderPosition == -1 || sliderPosition < timeLineLeft){
		...
		styleColor = "#ff0000";
		timelnCtx.strokeStyle = styleColor;
		timelnCtx.lineTo(sliderPosition, timeLineTop);
		timelnCtx.stroke();
		timelnCtx.save();
		timelnCtx.beginPath();
		timelnCtx.moveTo(sliderPosition, timeLineTop);
	...
	while(timeRefTable.next()){
		...
		for(var i=0;i<recordedObjArr.length;i++){
			...
			var objectTable = pageObjTable.get(pageNum);
			var graphicsObj = objectTable.get(objId);
			//console.log.log.log("graphicsObj Type::"+graphicsObj.type);
			var timeLinePos = Math.floor((lineLength * objTime)/recFileDuration);
			if(timeLinePos == 0){
				timeLinePos = timeLineLeft;
			}
	...
	sliderPointsArray[3] = ye;	
	createSliderOval(x0,y0,xe,ye,"#000000");
	//alert("timeRefTable:::"+timeRefTable);
}
...
//alert("oval"+styleColor);
	//console.log.log.log("activePage:;"+activePage+":pageNum:"+pageNum);
	timelnCtx.save();
	...
	timelnCtx.strokeStyle = styleColor;
	timelnCtx.beginPath();
	timelnCtx.moveTo(x+rx,y+ry);
	//console.log.log.log("x:"+x+":y:"+(y+ry));
	timelnCtx.lineTo(x+rx,y+2*ry-2);
	timelnCtx.moveTo(x,y);
	timelnCtx.stroke();
	
    timelnCtx.beginPath();
	timelnCtx.translate(x + rx, y +pageCircleDistfrmLine + ry);
    rx = Math.abs(rx);
    ry = Math.abs(ry);
    if (rx < ry){
...
function createAnnotatedObjOnTimeLine(x, y, xe, ye,styleColor,pageNum){
//alert("oval"+styleColor);
	//console.log.log.log("activePage:;"+activePage+":pageNum:"+pageNum);
	timelnCtx.save();
	timelnCtx.lineWidth = 1;
...
function findPos(obj) {
	...
	//console.log.log.log("findPos::::"+obj.offsetParent);
	if (obj.offsetParent) {
		do {
		   curleft += obj.offsetLeft;
		   curtop += obj.offsetTop;
		} while (obj = obj.offsetParent);
		//console.log.log.log("curleft::"+curleft+"::curtop:"+curtop);
...
function handleMouseMoveOnTimeLine(e){
	...
	//console.log.log("timelnOffsetX::"+timelnOffsetX+"::timelnOffsetY::"+timelnOffsetY);
	//console.log.log("x:"+mouseX+":y:"+mouseY+"sliderlength::"+sliderLength);
//	var status = checkPointsInSlider(mouseX,mouseY);
//	//console.log.log.log("Status:;"+status+":::sliderDrag::"+sliderDrag);
	if(sliderDrag){
		
		translateSliderOnTimeLine(mouseX,mouseY)
	}else{
	//console.log.log.log("x:"+mouseX+":y:"+mouseY);
		var objectTime = checkPointsOnTimeLineObj(mouseX,mouseY);
		//console.log.log.log("Time::::"+objectTime);
		selObjArray = null;
		selObjArray = new Array();
		if(objectTime != -1){
			...
			for(var i=0;i<recordedObjArr.length;i++){
			 ...
			 if(selObjArray!= null){
				...
				//console.log.log.log("lastDrawnObjTime::"+lastDrawnObjTime+":;objectTime::"+objectTime);
...
function handleMousedownOnTimeLine(e){
	var mouseX = parseInt(e.clientX - timelnOffsetX);
	var mouseY = parseInt(e.clientY - timelnOffsetY);
	//console.log.log.log("mouseX::"+mouseX+"mouseY::"+mouseY);	
	var status = checkPointsInSlider(mouseX,mouseY);
	if(status){
		sliderDrag = true;
	}else{
...
function handleClickOnTimeLine(e){
	...
	//console.log.log.log("x:"+mouseX+":y:"+mouseY);
	var objectTime = checkPointsOnTimeLineObj(mouseX,mouseY);
	//console.log.log.log("Time on click::::"+objectTime);
	selObjArray = null;
	selObjArray = new Array();
	if(objectTime != -1){
		clearTimeout(glowTimerId);
		...
		for(var i=0;i<recordedObjArr.length;i++){
			...
			if(pageNum == document.getElementById("pagenum").value){
				...
				lastDrawnObjTime = objectTime;
				playFronObjId = objId;
				//console.log.log.log("objId:::::::::::::::::::::::"+objId+"::selObjArray:"+selObjArray);
				//
			}else{
	...
	if(timeLineObjArray != null){
		for(var i=0;i<timeLineObjArray.length;i++){
			...
			if(timeLineObject.type == 1){
				...
				//console.log.log.log("Points Array::"+pointsArr);				
				/*
				var polyArr = new Array();
				var pt1 = new points(pointsArr[0],pointsArr[1]);
				var pt2 = new points(pointsArr[2],pointsArr[3]);
				...
				var pt = new points(mouseX,mouseY);
						
				
				if(isPointInPoly(polyArr,pt)){
					//console.log.log.log("Points in Polygon");
					//alert("In in Poly");
					objTime = timeLineObject.timeRef;
					break;
				}*/
				if(is_in_ellipse(mouseX,mouseY,pointsArr[2],pointsArr[3]+pageCircleDistfrmLine,pointsArr[6],pointsArr[7]+pageCircleDistfrmLine)){
					//console.log.log.log("Points in Circle");
					//alert("In in Circle");
					objTime = timeLineObject.timeRef;
					break;
				}
			
			}else if(timeLineObject.type == 2){
				var pointsArr = timeLineObject.pointsArray;
				var polyArr = new Array();
				var pt1 = new points(pointsArr[0],pointsArr[1]);
				var pt2 = new points(pointsArr[2],pointsArr[3]);
				...
				/*
				if(is_in_ellipse(mouseX,mouseY,pointsArr[2],pointsArr[3],pointsArr[6],pointsArr[7])){
					//console.log.log.log("Points in Circle");
					//alert("In in Circle");
					objTime = timeLineObject.timeRef;
					break;
...
function translateSliderOnTimeLine(mouseX,mouseY){
	if(mouseX <= sliderLength && mouseX >= timeLineLeft){
	//console.log.log.log("translateSliderOnTimeLine::;"+mouseX);
...
function glowObject(){
	//console.log.log.log("glow object");
	ctx.save();
	ctx.shadowColor = '#00ff00';
	ctx.shadowBlur = 20;
	ctx.shadowOffsetX = 0;
