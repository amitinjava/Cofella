<!--
	...
	jQuery(document).ready(function(){
		...
		if(isMobDevice){
			//alert("isMobDevice  canvas.width :::"+canvas.width +"canvas.height"+canvas.height);
			canvas.width = (windowW - 80);
			canvas.height = (windowH - 96);
	...
	if(isDrawing){
		...
		if(currentObjId != 7){
			...
			if(currentObjId != 16){
				...
				var graphicsObject;
				if(currentObjId == 25){
					graphicsObject = new GraphicsObject(count,currentObjId,tempPointsArray,lineWidth,heighlighterColor,null,isFilled,fillColor,heighlighterOpacity,false,null,null,1,null);
				}else{
...
function GraphicsObject(id,type,pointsArray,lnWidth,lnColor,src,isFilled,fillColor,opacity,imageLoaded,textObj,ref,isDraft, sequenceNo){
	this.id = id;
	this.type = type;
...
function saveTextAsGraphicsObject(text){
		...
		if(selectedTxtObj != null){
			...
			for(var ctr=0;ctr < text.length ;ctr++){
				...
				graphicsObject = new GraphicsObject(count,currentObjId,tempPointsArray,lineWidth,textStyleColor,null,false,fillColor,globalAlpha,false,textObj,null,1,null);
...
function setStyleColor(selectedColor){
	console.log("setStyleColor::"+selectedColor)
...
function openMeeting(meetingName){
	...
	var draftMsgId = document.getElementById("draftMsgId").value;
	var paramdata = "meetingName="+meetingName+"&status="+true+"&channel_id="+channelId+"&userId="+userId;
	if(draftMsgId != null){
		  		...
		  		success:function(msg) {
		  			...
		  			if(msg.actionErrors != null)
		  			{
		  				alert("Either new message or not found in DB");
		  		  		...
		  		  		if(json.meetingContextSequence != null && json.meetingContextSequence != ''){
		  		  			// draft message
						...
						if(json.RecFname!= null){
		  		  		...
		  		  		if(json.PageList != null){
		  		  			parseMeetingJSON(json.PageList,currentSeqNumber);
		  		  		}
...
function getCanvasObject(sequenceNum){
	...
	alert("meetingName -:::- "+meetingName);
	 jQuery.ajax({
		  	type:	"get",
	  		url: 	"getmeetingjson.action",
	  		data: "meetingName="+meetingName+"&status="+true+"&channel_id="+channel_id+"&participantId= "+sequenceNum+"&userId="+userId,
	  		contentType: 'application/json; charset=utf-8',
	  		dataType: 'json',
		  		success:function(msg) {
		  			//alert(msg.parentId);
...
function pasteCopiedObject(){
	...
	for(var i=0;i<copiedObjArr.length;i++){
		...
		var newGraphicsObject = new GraphicsObject(count,graphicsObject.type,newObjPtsArray,graphicsObject.lineWidth,graphicsObject.color,graphicsObject.src,graphicsObject.isFilled,graphicsObject.fillColor,graphicsObject.opacity,graphicsObject.imageLoaded,textObj,graphicsObject.ref,graphicsObject.isDraft,graphicsObject.sequenceNo);
...
function setRecordingFilePath(recfilePath){
	alert("setRecordingFilePath:::"+setRecordingFilePath);
