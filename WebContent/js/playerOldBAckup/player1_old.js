<!--
	...
	//	sliderDiv.innerHTML = '<input id="ex1" data-slider-id="ex1Slider" type="text" data-slider-min="0" data-slider-max="'+(windowW - 90)+'" data-slider-step="1" data-slider-value="14" style="width:'+(windowW - 90)+'px;"/>';
		var myElement = document.getElementById("timeline"); 
		//alert("myelement:;"+myElement.getBoundingClientRect().left);
		if(isMobDevice){
		...
		console.log("offsetY"+offsetY+":timelnOffsetY"+timelnOffsetY);
		initPlayer();
		...
		timelnCanvas.addEventListener("dragenter", function (evt) {
			console.log("timelnCanvas Drag Enter------------------");
			evt.preventDefault();
			//createDropZone();
		}, false);
		
		timelnCanvas.addEventListener("drag", function (evt) {
			console.log("timelnCanvas Drag Over------------------");
			evt.preventDefault();
			//createDropZone();
		}, false);
		
		timelnCanvas.addEventListener("dragleave", function (evt) {
			console.log("timelnCanvas Drag Leave------------------");
			evt.preventDefault();
			//createDropZone();
		}, false);
...
function moveSlider(){
	 ...
	 prevTime = cTime;
	//console.log("start slide::::"+sliderMoveTime+"::timeElap::"+timeElap);
	*/
	//console.log("start slide::::"+sliderPos);
	jQuery('#ex1').slider('setValue',sliderPos++);
	var lineLength =  window.innerWidth - 80;
	var timeLinePos = Math.floor((lineLength * myVid.currentTime * 1000)/recFileDuration);
	//console.log("timeLinePos::"+timeLinePos);
	if(timeLinePos >= timeLineLeft){
...
function glowSelectedObject(graphicsObject){
	...
	ctx.strokeStyle = "white";
	ctx.lineWidth= parseInt(graphicsObject.lineWidth) + 10;
	console.log("glowSelectedObject ::::::"+ctx.lineWidth);
	ctx.globalAlpha = 1.0;
/*	if(objectSel.type == 1){
...
function createTimeLine(){
	...
	var x0 = timeLineLeft;
	var y0 = timeLineTop - sliderRadius;
	var xe = timeLineLeft + 2*sliderRadius;
	var ye = timeLineTop + sliderRadius;
	...
	while(timeRefTable.next()){
		...
		for(var i=0;i<recordedObjArr.length;i++){
			...
			console.log("graphicsObj Type::"+graphicsObj.type+"recFileDuration::"+recFileDuration);
...
function refreshTimeLine(objectTime){
	//alert("timeRefTable:::"+refreshTimeLine);
	//console.log("refreshTimeLine::"+objectTime);
	if(objectTime != -1){
	...
	//console.log("lastDrawnObjTime:;"+lastDrawnObjTime+"::w::"+objectTime);
	...
	while(timeRefTable.next()){
		...
		for(var i=0;i<recordedObjArr.length;i++){
			...
			var objectTable = pageObjTable.get(pageNum);
			var graphicsObj = objectTable.get(objId);
			//console.log("graphicsObj Type::"+graphicsObj.type);
			var timeLinePos = Math.floor((lineLength * objTime)/recFileDuration);
			if(timeLinePos == 0){
				timeLinePos = timeLineLeft;
			}
	...
	if(timeLineObjArray != null){
		for(var i=0;i<timeLineObjArray.length;i++){
			...
			if(timeLineObject.type == 1){
				...
				//console.log("Points Array::"+pointsArr);				
				/*
				var polyArr = new Array();
				var pt1 = new points(pointsArr[0],pointsArr[1]);
				var pt2 = new points(pointsArr[2],pointsArr[3]);
