...
(function () {
    ...
    YAHOO.extend(YAHOO.widget.Toolbar, YAHOO.util.Element, {
        ...
        _addMenuClasses: function (H, E, I) {
        ...
        initAttributes: function (E) {
            ...
            this.setAttributeConfig("grouplabels", {
                value: ((E.grouplabels === false) ? false : true),
                method: function (F) {
                    if (F) {
        ...
        _createColorPicker: function (H) {
            ...
            A.on(E, "mousedown", function (J) {
                ...
                var I = A.getTarget(J);
                if (I.tagName.toLowerCase() == "a") {
                    var L = this.fireEvent("colorPickerClicked", {
                        type: "colorPickerClicked",
                        target: this,
                        button: this._colorPicker._button,
                        color: I.innerHTML,
        ...
        _makeColorButton: function (E) {
            ...
            E.type = "color";
            E.menu = new YAHOO.widget.Overlay(this.get("id") + "_" + E.value + "_menu", {
                visible: false,
                position: "absolute",
                iframe: true
            });
        ...
        _makeSpinButton: function (R, L) {
            ...
            var S = this,
                N = R._button.parentNode.parentNode,
                I = L.range,
            ...
            N.appendChild(H);
            N.appendChild(G);
            var M = YAHOO.lang.substitute(this.STR_SPIN_LABEL, {
                VALUE: R.get("label")
            });
            R.set("title", M);
            var Q = function (T) {
        ...
        _buttonClick: function (L, F) {
            ...
            if (L && L.type == "keypress") {
                if (L.keyCode == 9) {
                    ...
                    if ((L.keyCode === 13) || (L.keyCode === 0) || (L.keyCode === 32)) {} else {
                        E = false;
                    }
                }
            }
            if (E) {
                ...
                if (F.value) {
                    H = this.fireEvent(F.value + "Click", {
                        type: F.value + "Click",
                        target: this.get("element"),
                        button: F
                    });
                ...
                if (F.menucmd && N) {
                    H = this.fireEvent(F.menucmd + "Click", {
                        type: F.menucmd + "Click",
                        target: this.get("element"),
                        button: F
                    });
                    if (H === false) {
        ...
        destroy: function () {
            var F = this._configuredButtons.length,
                G, H, E;
            for (E = 0; E < F; E++) {
                this.destroyButton(this._configuredButtons[E]);
            }
            this._configuredButtons = null;
            this.get("element").innerHTML = "";
            this.get("element").className = "";
            for (H in this) {
                if (D.hasOwnProperty(this, H)) {
                    this[H] = null;
                }
            }
            return true;
        ...
        collapse: function (F) {
            ...
            if (F === false) {
                ...
                this.fireEvent("toolbarExpanded", {
...
(function () {
    ...
    YAHOO.widget.SimpleEditor = function (H, M) {
        ...
        I.attributes.element_cont = new YAHOO.util.Element(J, {
            id: F + "_container"
        });
        var E = document.createElement("div");
    ...
    YAHOO.extend(YAHOO.widget.SimpleEditor, YAHOO.util.Element, {
        ...
        _setupResize: function () {
            ...
            if (this.get("resize")) {
                ...
                this.resize.on("resize", function (G) {
                    ...
                    var H = G.height,
                        I = (this.toolbar.get("element").clientHeight + 2),
                        J = 0;
                    if (this.dompath) {
        ...
        _toggleDesignMode: function () {
            var F = this._getDoc().designMode,
                E = ((F.toLowerCase() == "on") ? "off" : "on");
        ...
        _handleFocus: function (E) {
            if (!this._focused) {
                this._focused = true;
                this.fireEvent("editorWindowFocus", {
                    type: "editorWindowFocus",
                    target: this
                });
            }
        },
        _handleBlur: function (E) {
            if (this._focused) {
                this._focused = false;
                this.fireEvent("editorWindowBlur", {
                    type: "editorWindowBlur",
                    target: this
                });
            }
        },
        ...
        _initEditor: function (F) {
            ...
            if (F) {
                this.fireEvent("editorContentReloaded", {
                    type: "editorreloaded",
                    target: this
                });
            } else {
        ...
        _setInitialContent: function (F) {
            var I = ((this._textarea) ? this.get("element").value : this.get("element").innerHTML),
                K = null;
            if (I === "") {
                I = "<br>";
            }
            var G = D.substitute(this.get("html"), {
                    TITLE: this.STR_TITLE,
                    CONTENT: this._cleanIncomingHTML(I),
                    CSS: this.get("css"),
            ...
            if (document.compatMode != "BackCompat") {
                G = this._docType + "\n" + G;
            } else {} if (this.browser.ie || this.browser.webkit || this.browser.opera || (navigator.userAgent.indexOf("Firefox/1.5") != -1)) {
                try {
                    if (this.browser.air) {
        ...
        _handleClick: function (G) {
            var F = this.fireEvent("beforeEditorClick", {
                type: "beforeEditorClick",
                target: this,
                ev: G
            });
            if (F === false) {
        ...
        _handleMouseUp: function (G) {
            var F = this.fireEvent("beforeEditorMouseUp", {
                type: "beforeEditorMouseUp",
                target: this,
                ev: G
            });
            if (F === false) {
        ...
        _handleMouseDown: function (F) {
            var E = this.fireEvent("beforeEditorMouseDown", {
                type: "beforeEditorMouseDown",
                target: this,
                ev: F
            });
            if (E === false) {
            ...
            this.fireEvent("editorMouseDown", {
                type: "editorMouseDown",
                target: this,
                ev: F
            });
        },
        _handleDoubleClick: function (F) {
            var E = this.fireEvent("beforeEditorDoubleClick", {
            ...
            if (this._isElement(G, "img")) {
                ...
                this.toolbar.fireEvent("insertimageClick", {
                    type: "insertimageClick",
                    target: this.toolbar
                });
                ...
                if (this._hasParent(G, "a")) {
                    ...
                    this.toolbar.fireEvent("createlinkClick", {
                        type: "createlinkClick",
                        target: this.toolbar
                    });
                    this.fireEvent("afterExecCommand", {
                        type: "afterExecCommand",
            ...
            this.nodeChange();
            this.fireEvent("editorDoubleClick", {
        ...
        _handleKeyUp: function (G) {
            var F = this.fireEvent("beforeEditorKeyUp", {
                type: "beforeEditorKeyUp",
                target: this,
                ev: G
            });
            if (F === false) {
        ...
        _handleKeyPress: function (G) {
            var F = this.fireEvent("beforeEditorKeyPress", {
                type: "beforeEditorKeyPress",
                target: this,
                ev: G
            });
            if (F === false) {
        ...
        _handleKeyDown: function (X) {
            var Z = this.fireEvent("beforeEditorKeyDown", {
                type: "beforeEditorKeyDown",
                target: this,
                ev: X
            });
            if (Z === false) {
            ...
            case this._keyMap.FONT_SIZE_DOWN.key:
                ...
                if (K || T) {
                    var H = this.toolbar.getButtonByValue("fontsize"),
                        G = parseInt(H.get("label"), 10),
                        I = (G + 1);
                    if (T) {
                        I = (G - 1);
                    }
                    ...
                    if (this.browser.webkit) {
                        ...
                        if (!this._hasParent(U, "li")) {
                            if (this.browser.webkit4) {
                                ...
                                var L = this._getDoc().getElementById("yui-br"),
                                    Y = this._getDoc().createElement("br"),
                                    J = this._getDoc().createElement("span");
        ...
        _fixListDupIds: function () {
            ...
            if (this._getDoc()) {
                ...
                var E = this._getDoc().body.getElementsByTagName("li"),
                    F = 0,
                    G = {};
        ...
        _nodeChange: function (F) {
            var H = parseInt(this.get("nodeChangeThreshold"), 10),
                O = Math.round(new Date().getTime() / 1000),
                R = this;
            if (F === true) {
            ...
            var Z = this.fireEvent("beforeNodeChange", {
                type: "beforeNodeChange",
                target: this
            });
            if (Z === false) {
                return false;
            ...
            if (!this.get("disabled")) {
                if (this.STOP_NODE_CHANGE) {
                    ...
                    var T = this._getSelection(),
                        Q = this._getRange(),
                        E = this._getSelectedElement(),
                        M = this.toolbar.getButtonByValue("fontname"),
                        L = this.toolbar.getButtonByValue("fontsize"),
                        J = this.toolbar.getButtonByValue("undo"),
                        G = this.toolbar.getButtonByValue("redo");
                    var N = {};
                    if (this._lastButton) {
                    ...
                    for (var b = 0; b < this._disabled.length; b++) {
                        var P = this.toolbar.getButtonByValue(this._disabled[b]);
                        if (P && P.get) {
                            if (this._lastButton && (P.get("id") === this._lastButton.id)) {} else {
                                if (!this._hasSelection() && !this.get("insert")) {
                                    switch (this._disabled[b]) {
                                    case "fontname":
                                    case "fontsize":
                                        break;
                                    default:
                                    ...
                                    if (!this._alwaysDisabled[this._disabled[b]]) {
                                        this.toolbar.enableButton(P);
                                    }
                                } if (!this._alwaysEnabled[this._disabled[b]]) {
                    ...
                    for (var X = 0; X < S.length; X++) {
                        c = S[X].tagName.toLowerCase();
                        if (S[X].getAttribute("tag")) {
                            c = S[X].getAttribute("tag").toLowerCase();
                        }
                        W = this._tag2cmd[c];
                        if (W === undefined) {
                            W = [];
            ...
            this.fireEvent("afterNodeChange", {
                type: "afterNodeChange",
                target: this
            });
        },
        ...
        initAttributes: function (E) {
            ...
            this.setAttributeConfig("setDesignMode", {
                value: ((E.setDesignMode === false) ? false : true)
            });
            this.setAttributeConfig("nodeChangeDelay", {
                value: ((E.nodeChangeDelay === false) ? false : true)
            });
            ...
            this.setAttributeConfig("container", {
                writeOnce: true,
                value: E.container || false
            });
            this.setAttributeConfig("plainText", {
                writeOnce: true,
                value: E.plainText || false
            });
            ...
            this.setAttributeConfig("nodeChangeThreshold", {
                value: E.nodeChangeThreshold || 3,
                validator: YAHOO.lang.isNumber
            });
            this.setAttributeConfig("allowNoEdit", {
                value: E.allowNoEdit || false,
                validator: YAHOO.lang.isBoolean
            });
            this.setAttributeConfig("limitCommands", {
                value: E.limitCommands || false,
                validator: YAHOO.lang.isBoolean
            });
            this.setAttributeConfig("element_cont", {
                value: E.element_cont
            });
            this.setAttributeConfig("editor_wrapper", {
                value: E.editor_wrapper || null,
                writeOnce: true
            });
            this.setAttributeConfig("height", {
                value: E.height || C.getStyle(F.get("element"), "height"),
                method: function (G) {
                    if (this._rendered) {
                        if (this.get("animate")) {
                            var H = new YAHOO.util.Anim(this.get("iframe").get("parentNode"), {
                                height: {
                                    to: parseInt(G, 10)
                                }
                            }, 0.5);
            ...
            this.setAttributeConfig("width", {
                value: E.width || C.getStyle(this.get("element"), "width"),
                method: function (G) {
                    if (this._rendered) {
                        if (this.get("animate")) {
                            var H = new YAHOO.util.Anim(this.get("element_cont").get("element"), {
                                width: {
                                    to: parseInt(G, 10)
                                }
                            }, 0.5);
            ...
            this.setAttributeConfig("blankimage", {
                value: E.blankimage || this._getBlankImage()
            });
            this.setAttributeConfig("css", {
                value: E.css || this._defaultCSS,
                writeOnce: true
            });
            this.setAttributeConfig("html", {
                value: E.html || '<html><head><title>{TITLE}</title><meta http-equiv="Content-Type" content="text/html; charset=UTF-8" /><base href="' + this._baseHREF + '"><style>{CSS}</style><style>{HIDDEN_CSS}</style><style>{EXTRA_CSS}</style></head><body onload="document.body._rteLoaded = true;">{CONTENT}</body></html>',
                writeOnce: true
            });
            ...
            this.setAttributeConfig("toolbar", {
                value: E.toolbar || this._defaultToolbar,
                writeOnce: true,
                method: function (G) {
                    if (!G.buttonType) {
            ...
            this.setAttributeConfig("animate", {
                value: ((E.animate) ? ((YAHOO.util.Anim) ? true : false) : false),
            ...
            this.setAttributeConfig("removeLineBreaks", {
                value: E.removeLineBreaks || false,
                validator: YAHOO.lang.isBoolean
            });
            ...
            this.setAttributeConfig("resize", {
                writeOnce: true,
                value: E.resize || false
            });
            this.setAttributeConfig("filterWord", {
                value: E.filterWord || false,
                validator: YAHOO.lang.isBoolean
            });
        },
        ...
        _handleFormButtonClick: function (F) {
            var E = A.getTarget(F);
            this._formButtonClicked = E;
        },
        _handleFormSubmit: function (H) {
            this.saveHTML();
            var G = this.get("element").form,
                E = this._formButtonClicked || false;
        ...
        _handleAfterNodeChange: function () {
            var Q = this._getDomPath(),
                L = null,
                H = null,
                M = null,
                F = false,
                J = this.toolbar.getButtonByValue("fontname"),
                K = this.toolbar.getButtonByValue("fontsize"),
                E = this.toolbar.getButtonByValue("heading");
        ...
        execCommand: function (G, F) {
            var J = this.fireEvent("beforeExecCommand", {
                type: "beforeExecCommand",
                target: this,
                args: arguments
            });
            ...
            if (!this.browser.webkit) {
                ...
                if (G && this._isElement(G, "span") && this._hasSelection()) {
                    if (G.style.fontWeight == "bold") {
                        G.style.fontWeight = "";
                        var E = this._getDoc().createElement("b"),
                            F = G.parentNode;
                        F.replaceChild(E, G);
                        E.appendChild(G);
                    }
            ...
            if (!this.browser.webkit) {
                ...
                if (G && this._isElement(G, "span") && this._hasSelection()) {
                    if (G.style.fontStyle == "italic") {
                        G.style.fontStyle = "";
                        var E = this._getDoc().createElement("i"),
                            F = G.parentNode;
                        F.replaceChild(E, G);
                        E.appendChild(G);
                    }
        ...
        cmd_list: function (d) {
            ...
            if ((this.browser.webkit && !this.browser.webkit4) || (this.browser.opera)) {
                if (this._isElement(c, "li") && this._isElement(c.parentNode, d)) {
                    ...
                    this._createCurrentElement(d.toLowerCase());
                    Z = this._getDoc().createElement(d);
                    ...
                    var T = ((this.browser.opera) ? "<BR>" : "<br>"),
                        S = Z.firstChild.innerHTML.split(T),
                        Y, a;
                    if (S.length > 0) {
                        Z.innerHTML = "";
                ...
                if (this._isElement(G, "li") && this._isElement(G.parentNode, d) || (this.browser.ie && this._isElement(this._getRange().parentElement, "li")) || (this.browser.ie && this._isElement(G, "ul")) || (this.browser.ie && this._isElement(G, "ol"))) {
                    if (this.browser.ie) {
                        ...
                        var b = this._getDoc().createElement("span");
                        b.innerHTML = U;
                        G.parentNode.parentNode.replaceChild(b, G.parentNode);
                    } else {
                        this.nodeChange();
                ...
                if (this.browser.opera) {
                    var R = this;
                    window.setTimeout(function () {
                        var e = R._getDoc().getElementsByTagName("li");
                        for (var f = 0; f < e.length; f++) {
                            if (e[f].innerHTML.toLowerCase() == "<br>") {
                                e[f].parentNode.parentNode.removeChild(e[f].parentNode);
                            }
                        }
                    }, 30);
                }
                if (this.browser.ie && V) {
                    ...
                    this._getRange().pasteHTML("<" + d + ">" + L + "</" + d + ">");
        ...
        cmd_fontname: function (H) {
            ...
            if (G && G.tagName && !this._hasSelection() && !this._isElement(G, "body") && !this.get("insert")) {
                ...
                if (this.get("insert") && !this._hasSelection()) {
                    var F = this._createInsertElement({
                        fontFamily: H
                    });
                    E = false;
                }
        ...
        setEditorHTML: function (F) {
			...
			console.log("this._getDoc().body::::"+this._getDoc().body);
        ...
        cleanHTML: function (H) {
            ...
            if ((F == "semantic") || (F == "xhtml")) {
                H = H.replace(/<i([^>]*)>/gi, "<em$1>");
                H = H.replace(/<\/i>/gi, "</em>");
                H = H.replace(/<b([^>]*)>/gi, "<strong$1>");
            ...
            if (this.browser.ie) {
                ...
                var G = this._baseHREF.replace(/\//gi, "\\/"),
                    I = new RegExp('src="' + G, "gi");
                H = H.replace(I, 'src="');
            }
...
(function () {
    ...
    YAHOO.extend(YAHOO.widget.Editor, YAHOO.widget.SimpleEditor, {
        ...
        _undoNodeChange: function () {
            var E = this.toolbar.getButtonByValue("undo"),
                F = this.toolbar.getButtonByValue("redo");
            if (E && F) {
            ...
            if (!this._defaultImageToolbarConfig) {
                this._defaultImageToolbarConfig = {
                    ...
                    buttons: [{
                        ...
                        group: "border",
                        label: this.STR_IMAGE_BORDER + ":",
                        buttons: [{
                            type: "select",
                            label: this.STR_IMAGE_BORDER_SIZE,
                            value: "bordersize",
                            menu: [{
                                text: "none",
                                value: "0",
                            ...
                            type: "select",
                            label: this.STR_IMAGE_BORDER_TYPE,
                            value: "bordertype",
                            disabled: true,
                            menu: [{
                                text: "Solid",
        ...
        _renderCreateLinkWindow: function () {
            ...
            this.fireEvent("windowCreateLinkRender", {
                type: "windowCreateLinkRender",
        ...
        _handleCreateLinkClick: function () {
            ...
            this.on("afterExecCommand", function () {
                var K = new YAHOO.widget.EditorWindow("createlink", {
                    width: "350px"
                });
        ...
        _handleCreateLinkWindowClose: function () {
            var G = C.get(this.get("id") + "_createlink_url"),
                I = C.get(this.get("id") + "_createlink_target"),
                K = C.get(this.get("id") + "_createlink_title"),
                H = arguments[0].win.el,
                E = H;
            if (G && G.value) {
                var J = G.value;
        ...
        _renderInsertImageWindow: function () {
            ...
            H.on("colorPickerClicked", function (T) {
                var P = "1",
                    S = "solid",
                    O = "black",
                    R = this._defaultImageToolbar.editor_el;
            ...
            H.on("buttonClick", function (V) {
                var T = V.button.value,
                    S = this._defaultImageToolbar.editor_el,
                    R = "";
                if (V.button.menucmd) {
                    T = V.button.menucmd;
                }
            ...
            A.on(this.get("id") + "_insertimage_url", "blur", function () {
                var Q = C.get(this.get("id") + "_insertimage_url"),
                    R = this.currentElement[0];
                if (Q.value && R) {
            ...
            this.fireEvent("windowInsertImageRender", {
                type: "windowInsertImageRender",
                panel: this.get("panel"),
                body: K,
                toolbar: H
            });
        ...
        _handleInsertImageClick: function () {
            ...
            this.on("afterExecCommand", function () {
                var H = this.currentElement[0],
                    ...
                    J = 0,
                    S = false,
                    I = new YAHOO.widget.EditorWindow("insertimage", {
                        width: "415px"
                    });
                if (!H) {
                    H = this._getSelectedElement();
                ...
                var Y = G.getButtonByValue("bordersize"),
                    W = ((parseInt(F, 10) > 0) ? "" : this.STR_NONE);
        ...
        _handleInsertImageWindowClose: function () {
            var E = C.get(this.get("id") + "_insertimage_url"),
                L = C.get(this.get("id") + "_insertimage_title"),
                I = C.get(this.get("id") + "_insertimage_link"),
                J = C.get(this.get("id") + "_insertimage_target"),
                H = arguments[0].win.el;
        ...
        _renderPanel: function () {
            ...
            var E = new YAHOO.widget.Overlay(this.get("id") + this.EDITOR_PANEL_ID, {
                width: "300px",
                iframe: true,
                visible: false,
                underlay: "none",
        ...
        openWindow: function (K) {
            ...
            var Q = C.getXY(this.currentElement[0]),
                N = C.getXY(this.get("iframe").get("element")),
                E = this.get("panel"),
                H = [(Q[0] + N[0] - 20), (Q[1] + N[1] + 10)],
                G = (parseInt(K.attrs.width, 10) / 2),
                L = "center",
                J = null;
            this.fireEvent("beforeOpenWindow", {
                type: "beforeOpenWindow",
                win: K,
                panel: E
            });
            var F = E.editor_form;
            ...
            E.show();
            this.fireEvent("afterOpenWindow", {
                type: "afterOpenWindow",
                win: K,
                panel: E
            });
        },
        moveWindow: function (F) {
            ...
            var I = this.currentWindow,
                J = C.getXY(this.currentElement[0]),
                a = C.getXY(this.get("iframe").get("element")),
                O = this.get("panel"),
                Y = [(J[0] + a[0]), (J[1] + a[1])],
                R = (parseInt(I.attrs.width, 10) / 2),
                U = "center",
                Q = O.cfg.getProperty("xy") || [0, 0],
                G = O.editor_knob,
                X = 0,
                L = 0,
                T = false;
        ...
        closeWindow: function (E) {
            this.fireEvent("window" + this.currentWindow.name + "Close", {
                type: "window" + this.currentWindow.name + "Close",
...
YAHOO.register("editor", YAHOO.widget.Editor, {
    version: "2.9.0",
    build: "2800"
});